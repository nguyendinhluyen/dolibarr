...
// DB Nanapet
$object = new Product($db, $db_nanapet);
...
if (empty($reshook)) {
    ...
    if ($action == 'add' && ($user->rights->produit->creer || $user->rights->service->creer)) {
        $error = 0;
        if (empty($ref)) {
            setEventMessage($langs->trans('ErrorFieldRequired', $langs->transnoentities('Sku')), 'errors');
            $action = "create";
            $error++;
        }
        if (!GETPOST('label')) {
            setEventMessage($langs->trans('ErrorFieldRequired', $langs->transnoentities('ProductName')), 'errors');
            $action = "create";
            $error++;
        }
        ...
        if (!$error) {
            ...
            /* NanaPet */
            $object->manufacturer = GETPOST('manufacturer');
            $object->sellingPriceNanaPet = GETPOST('sellingPriceNanaPet');
            $object->discountPriceNanaPet = GETPOST('discountPriceNanaPet');
            $object->status_product = GETPOST('status_units');             
            
            if (isset($_POST['chkDog']) && !isset($_POST['chkCat'])) {
                $object->species = "10";
            } else if (!isset($_POST['chkDog']) && isset($_POST['chkCat'])) {
                $object->species = "01";
            } else if (isset($_POST['chkDog']) && isset($_POST['chkCat'])) {
                $object->species = "11";
            }            
            $object->keywords = GETPOST('keywords');
            /* End NanaPet */
    ...
    if ($action == 'update' && ($user->rights->produit->creer || $user->rights->service->creer)) {
        if (GETPOST('cancel')) {
            ...
            if ($object->id > 0) {
                ...
                //$object->status = GETPOST('statut');
                //$object->status_buy = GETPOST('statut_buy');
                $object->status = "1"; // NanaPet online sell
                $object->status_buy="0";
                $object->status_batch = GETPOST('status_batch');
        ...
        if ($thirpdartyid > 0) {
            ...
            if (GETPOST('propalid') > 0) {
                $result = $propal->addline(
                        ...
                        $object->id, GETPOST('remise_percent'), $price_base_type, $pu_ttc, 0, 0, -1, 0, 0, 0, 0, '', '', '', 0, $object->fk_unit
                );
                if ($result > 0) {
                ...
                $result = $commande->addline(
                        $desc, $pu_ht, GETPOST('qty'), $tva_tx, $localtax1_tx, // localtax1
                        $localtax2_tx, // localtax2
                        $object->id, GETPOST('remise_percent'), '', '', $price_base_type, $pu_ttc, '', '', 0, -1, 0, 0, null, 0, '', 0, $object->fk_unit
                );
                ...
                $result = $facture->addline(
                        $desc, $pu_ht, GETPOST('qty'), $tva_tx, $localtax1_tx, $localtax2_tx, $object->id, GETPOST('remise_percent'), '', '', '', '', '', $price_base_type, $pu_ttc, Facture::TYPE_STANDARD, -1, 0, '', 0, 0, null, 0, '', 0, 100, '', $object->fk_unit
                );
...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
    ...
    if ($action == 'create' && ($user->rights->produit->creer || $user->rights->service->creer)) {
        ...
        print '<td class="fieldrequired" width="20%">' . $langs->trans("Sku") . '</td><td colspan="3"><input name="ref" size="32" maxlength="128" value="' . dol_escape_htmltag(GETPOST('ref') ? GETPOST('ref') : $tmpcode) . '">';
        if ($refalreadyexists) {
        ...
        // Label
        print '<tr><td class="fieldrequired">' . $langs->trans("ProductName") . '</td><td colspan="3"><input name="label" size="40" maxlength="255" value="' . dol_escape_htmltag(GETPOST('label')) . '"></td></tr>';
        /* NanaPet */
        print '<tr><td >' . $langs->trans("BrandName") . '</td><td colspan="3"><input name="manufacturer" size="40" maxlength="255" value="' . dol_escape_htmltag(GETPOST('manufacturer')) . '"></td></tr>';
        // Custom code
        ...
        print '<tr><td>' . $langs->trans("UseFor") . '</td>'
                . '<td style="width: 200px"><input size ="50" type = "checkbox" name = "chkDog" value ="1">' . $langs->trans("Dog") . '</td>'
                . '<td><input size ="50" type = "checkbox" name = "chkCat" value ="2">' . $langs->trans("Cat") . '</td>'
                . '</tr>';
        print '<tr><td>' . $langs->trans("SellingPriceNanaPet") . '</td>';
        print '<td><input name="sellingPriceNanaPet" size="10" value="' . $object->sellingPriceNanaPet . '"> ' . $langs->trans("VND");
        print '</td></tr>';
        print '<tr><td>' . $langs->trans("DiscountPrice") . '</td>';
        print '<td><input name="discountPriceNanaPet" size="10" value="' . $object->discountPriceNanaPet . '"> ' . $langs->trans("VND");
        print '</td></tr>';
        print '<tr><td>' . $langs->trans("SellingPriceDolibarr") . '</td>';
        print '<td><input name="price" size="10" value="' . $object->price . '"> ' . $langs->trans("VND");
        //print $form->selectPriceBaseType($object->price_base_type, "price_base_type") . ' ' . $langs->trans("VND");
        print ' </td></tr>';
        print '<tr><td>' . $langs->trans("Status") . '</td><td colspan="3">';
        print $formproduct->select_measuring_units("status_units", "status");
        print '</td></tr>';
        print '<tr><td class="tdtop">' . $langs->trans("Keywords") . '</td><td colspan="3"><input name="keywords" size="40" maxlength="255" value="' . dol_escape_htmltag(GETPOST('keywords')) . '"></td></tr>';
        print "<br>";
        /* End NanaPet */
        ...
        dol_fiche_end();
        print '<div class="center"><input type="submit" class="button" value="' . $langs->trans("Thêm mới") . '"></div>';
        print '</form>';
    }
     ...
     */ else if ($object->id > 0) {
        ...
        if ($action == 'edit' && ($user->rights->produit->creer || $user->rights->service->creer)) {
            ...
            // Ref
            print '<tr><td width="20%" class="fieldrequired">' . $langs->trans("Sku") . '</td><td colspan="3"><input name="ref" size="32" maxlength="128" value="' . dol_escape_htmltag($object->ref) . '"></td></tr>';
            // Label
            print '<tr><td class="fieldrequired">' . $langs->trans("ProductName") . '</td><td colspan="3"><input name="label" size="40" maxlength="255" value="' . dol_escape_htmltag($object->label) . '"></td></tr>';

            /* NanaPet */
            print '<tr><td>' . $langs->trans("BrandName") . '</td><td colspan="3"><input name="manufacturer" size="40" maxlength="255" value="' . dol_escape_htmltag($object->manufacturer) . '"></td></tr>';
            // Original Country
            if (empty($conf->global->PRODUCT_DISABLE_CUSTOM_INFO)) {
                print '<td>' . $langs->trans("CountryOrigin") . '</td><td>';
            ...
            // Check box Dog and Cat
            if (dol_escape_htmltag($object->species) == "11") { // Dog and Cat
                print '<tr><td>' . $langs->trans("UseFor") . '</td>'
                        . '<td style="width: 200px"><input size ="50" type = "checkbox" name = "chkDog" value ="1" checked>' . $langs->trans("Dog") . '</td>'
                        . '<td><input size ="50" type = "checkbox" name = "chkCat" value ="2" checked>' . $langs->trans("Cat") . '</td>'
                        . '</tr>';
            } else if (dol_escape_htmltag($object->species) == "10") { // Dog
                print '<tr><td>' . $langs->trans("UseFor") . '</td>'
                        . '<td style="width: 200px"><input size ="50" type = "checkbox" name = "chkDog" value ="1" checked>' . $langs->trans("Dog") . '</td>'
                        . '<td><input size ="50" type = "checkbox" name = "chkCat" value ="2">' . $langs->trans("Cat") . '</td>'
                        . '</tr>';
            } else if (dol_escape_htmltag($object->species) == "01") { // Cat
                print '<tr><td>' . $langs->trans("UseFor") . '</td>'
                        . '<td style="width: 200px"><input size ="50" type = "checkbox" name = "chkDog" value ="1">' . $langs->trans("Dog") . '</td>'
                        . '<td><input size ="50" type = "checkbox" name = "chkCat" value ="2" checked>' . $langs->trans("Cat") . '</td>'
                        . '</tr>';
            ...
            print '<tr><td>' . $langs->trans("SellingPriceNanaPet") . '</td>';
            print '<td><input name="sellingPriceNanaPet" size="10" value="' . price2num($object->sellingPriceNanaPet) . '"> ' . $langs->trans("VND");
            print '</td></tr>';
            print '<tr><td>' . $langs->trans("DiscountPrice") . '</td>';
            print '<td><input name="discountPriceNanaPet" size="10" value="' . price2num($object->discountPriceNanaPet) . '"> ' . $langs->trans("VND");
            print '</td></tr>';
            print '<tr><td>' . $langs->trans("SellingPriceDolibarr") . '</td>';
            print '<td><input name="price" size="10" value="' . price2num($object->price) . '"> ' . $langs->trans("VND");
            //print $form->selectPriceBaseType($object->price_base_type, "price_base_type") . ' ' ;
            print ' </td></tr>';
            print '<tr><td>' . $langs->trans("Status") . '</td><td colspan="3">';
            print $formproduct->select_measuring_units("status_units", "status", $object->status_product);
            print '</td></tr>';
            print '<tr><td class="tdtop">' . $langs->trans("Keywords") . '</td><td colspan="3"><input name="keywords" size="40" maxlength="255" value="' . $object->keywords . '"></td></tr>';
            /* End NanaPet */
        ...
        // Show info PRODUCT
            ...
            // Ref
            print '<td width="15%">' . $langs->trans("Sku") . '</td><td colspan="' . (2 + (($showphoto || $showbarcode) ? 1 : 0)) . '">';
            print $form->showrefnav($object, 'ref', '', 1, 'ref');
            print '</td>';
            print '</tr>';
            // Label
            print '<tr><td>' . $langs->trans("ProductName") . '</td><td colspan="2">' . $object->label . '</td>';

            /* NanaPet View */
            print '<tr><td>' . $langs->trans("BrandName") . '</td><td colspan="2">' . $object->manufacturer . '</td>';
            print '<tr><td>' . $langs->trans("CountryOrigin") . '</td><td colspan="2">' . getCountry($object->country_id, 0, $db) . '</td>';
            print '<tr><td>' . $langs->trans("SellingPriceNanaPet") . '</td><td colspan="2">' . formatMoney(price2num($object->sellingPriceNanaPet)) . '</td>';
            print '<tr><td>' . $langs->trans("DiscountPrice") . '</td><td colspan="2">' . formatMoney(price2num($object->discountPriceNanaPet)) . '</td>';
            print '<tr><td>' . $langs->trans("SellingPriceDolibarr") . '</td><td colspan="2"><b style="color: red">' . formatMoney(price2num($object->price)) . '</b></td>';
            // Status 
            if ($object->status_product == 0) {
                $object->status_product = $langs->trans("StatusUnitConHang");
            } else if ($object->status_product == 3) {
                $object->status_product = $langs->trans("StatusUnitHetHang");
            } else if ($object->status_product == -3) {
                $object->status_product = $langs->trans("StatusUnitSapCoHang");
            } else if ($object->status_product == -6) {
                $object->status_product = $langs->trans("StatusUnitDatTruoc");
            }
            print '<tr><td>' . $langs->trans("Status") . '</td><td colspan="2"><b>' . $object->status_product . '</b></td>';
            /* End NanaPet */
