...
if (!$syncDbNanapet) {
    if($object->checkUpdateForProduct() == -1) {
        setEventMessage($langs->trans("WarningsProductDolibarr"), 'warnings');
    } else if ($object->checkUpdateForProduct() == 0) {
        setEventMessage($langs->trans("WarningsProductNanaPet"), 'warnings');
    }
}
$mesg_success=$langs->trans("syncDbNanaPetSuccess");
$mesg_error=$langs->trans("syncDbNanaPetError");
if($syncDbNanapet) {
    //$object->deleteAllProductDollibar();
    if ($object->syncProductsNanapet($user) > 0) {
...
print '<label for="sref">' . $langs->trans("Sku") . '</label>:</td><td><input class="flat" type="text" size="14" name="sref" id="sref"></td>';
...
print '<form method="post" action="' . DOL_URL_ROOT . '/product/index.php">';
print "<table><tr>";
print '<td rowspan="' . $rowspan . '"><input type="submit" name="syncDbNanapet" class="button" value="' . $langs->trans("syncDbNanapet") . '"></td></tr>';
print "</tr></table>";
print "</form>";
...
while ($objp = $db->fetch_object($result)) {
    ...
    if (!$objp->tosell && !$objp->tobuy)
        $status = 0; // To sell OR to buy
    if ((!$objp->tosell && $objp->tobuy) || ($objp->tosell && !$objp->tobuy))
        $status = 1;
