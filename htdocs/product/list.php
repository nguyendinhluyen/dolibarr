...
$warningStore = GETPOST("warningStore", 'int');
...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
    ...
    $sql.= ' p.duration, p.tosell, p.tobuy, p.seuil_stock_alerte, p.seuil_max_stock_alerte, p.desiredstock,';
    ...
    // Stock filter warning store
    if(isset($warningStore) && $warningStore == 0 && $warningStore!="") {
        $sql.= " AND IFNULL(CAST(p.seuil_max_stock_alerte AS SIGNED),0) < IFNULL(CAST(p.stock AS SIGNED),0) ";
    } else if(isset($warningStore) && $warningStore == 1 && $warningStore!="") {
        $sql.= " AND IFNULL(CAST(p.seuil_stock_alerte AS SIGNED),0) > IFNULL(CAST(p.stock AS SIGNED),0) ";
    }
    ...
    if ($resql) {
        ...
        if (!empty($canvas) && file_exists(DOL_DOCUMENT_ROOT . '/product/canvas/' . $canvas . '/actions_card_' . $canvas . '.class.php')) {
            ...
            // Lines titles
            print '<tr class="liste_titre">';
            print_liste_field_titre($langs->trans("Sku"), $_SERVER["PHP_SELF"], "p.ref", $param, "", "", $sortfield, $sortorder);
            print_liste_field_titre($langs->trans("ProductName"), $_SERVER["PHP_SELF"], "p.label", $param, "", "", $sortfield, $sortorder);
            if (!empty($conf->barcode->enabled)) {
            ...
            if (!empty($conf->service->enabled) && $type != 0) {
                print_liste_field_titre($langs->trans("Duration"), $_SERVER["PHP_SELF"], "p.duration", $param, "", 'align="center"', $sortfield, $sortorder);
            }
            /*
              if (empty($conf->global->PRODUIT_MULTIPRICES)) {
            ...
            /* NanaPet */
            $titlefields = $form->textwithpicto($langs->trans("SellingPrice") . " (dolibarr)", $langs->trans("DefaultPriceRealPriceMayDependOnCustomer"));
            print_liste_field_titre($titlefields, $_SERVER["PHP_SELF"], "p.price", $param, "", 'align="right"', $sortfield, $sortorder);
            print '<td class="liste_titre" align="right"> &nbsp</td>';
            /* End NanaPet */
            print '<td class="liste_titre" align="right">' . $langs->trans("") . '</td>';
            if (!empty($conf->stock->enabled) && $user->rights->stock->lire && $type != 1) {
                print '<td class="liste_titre" align="right">' . $langs->trans("") . '</td>';
            }
            if (!empty($conf->stock->enabled) && $user->rights->stock->lire && $type != 1) {
            ...
            print '<td class="liste_titre" align="right">';            
            print $form->selectarray('warningStore', 
                    array('0' => $langs->trans('WarningStoreLarger'),
                          '1' => $langs->trans('WarningStoreLess')), $warningStore, 1);
            print '</td >';
            ...
            while ($i < min($num, $limit)) {
                ...
                if (!empty($conf->service->enabled) && $type != 0) {
                    ...
                    if (preg_match('/([0-9]+)[a-z]/i', $objp->duration)) {
                        ...
                        else if (preg_match('/([0-9]+)w/i', $objp->duration, $regs)) {
                ...
                print formatMoney(price2num($objp->price)) ." ". $langs->trans("VND");                
                print '</td>';
                /* End NanaPet */
                ...
                if (!empty($conf->stock->enabled) && $user->rights->stock->lire && $type != 1) {
                    if ($objp->fk_product_type != 1) {
                        ...
                        } else if($product_static->stock_reel > $objp->seuil_max_stock_alerte){
