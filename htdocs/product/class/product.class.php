...
class Product extends CommonObject {
    /* NanaPet */
    var $manufacturer;
    var $sellingPriceNanaPet;
    var $discountPriceNanaPet;
    var $species;
    var $status_product;
    var $keywords;        
    var $id_dolibarr;
    var $ref_nanapet;
    var $products_id_nanapet;
    /* End NanaPet */
    ...
    function __construct($db, $dbNanapet=null) {
        ...
        $this->seuil_max_stock_alerte = 0;
        $this->desiredstock = 0;
        $this->canvas = '';
        $this->status_batch = 0;
        $this->manufacturer = '';    
        $this->ref_nanapet = 0;
        $this->sellingPriceNanaPet = 0;
        $this->discountPriceNanaPet = 0;
        $this->price = 0;
        $this->price_ttc = 0;
    }
     ...
     */
    function create($user, $notrigger = 0, $sync = false) {
        /* NanaPet */
        $this->manufacturer = trim($this->manufacturer);
        $this->sellingPriceNanaPet = price2num($this->sellingPriceNanaPet);
        $this->discountPriceNanaPet = price2num($this->discountPriceNanaPet);
        $this->species = trim($this->species);
        $this->status_product = trim($this->status_product);
        
        if(empty($this->sellingPriceNanaPet)) {
            $this->sellingPriceNanaPet = 0;
        }
        if(empty($this->discountPriceNanaPet)) {
            $this->discountPriceNanaPet = 0;
        }
        ...
        if ($result >= 0) {
            ...
            if ($result) {
                ...
                if ($obj->nb == 0) {
                    ...
                    $sql.= ", " . price2num($this->sellingPriceNanaPet);
                    $sql.= ", " . price2num($this->discountPriceNanaPet);
                    $sql.= ", '" . $this->species . "'";
                    $sql.= ", '" . price2num($this->status_product) . "'";
                    $sql.= ", '" . $this->keywords . "'";
                    $sql.= ", " . $this->ref_nanapet; 
                    $sql.= ")";                    
                    dol_syslog(get_class($this) . "::Create", LOG_DEBUG);
                    $result = $this->db->query($sql);
                    if ($result) {
                        $id = $this->db->last_insert_id(MAIN_DB_PREFIX . "product");
                        // Use to update ref_nanapet on Dolibbar
                        $this->id_dolibarr = $id;
                        if ($id > 0) {
                            $this->id = $id;
                            $this->price = $price_ht;
                            ...
                            if ($result > 0) {
                                if ($this->update($id, $user, true, 'add',true) <= 0) {
                                    $error++;
                                }
                            } else {
                                $error++;
            ...
            if (!$error) {          
                //Case: Not sync database NanaPet
                if($sync == false) {
                    if($this->insertProductNanapet()) {
                        $this->db->commit();
                        return $this->id;
                    } else {
                        $this->db->rollback();
                        $error++;
                        $this->error = 'Failed To Insert on Database NanaPet';
                        return -$error;
                    }
                } else {
                    //Case: insert data on Dolibarr
                    $this->db->commit();
                    return $this->id;
                }
     ...
     */
    function update($id, $user, $notrigger = false, $action = 'update', $flagInsert = false) {
        global $langs, $conf, $hookmanager;
        $error = 0;
        
        // DB Nanapet
        if ($action == 'updateNanapet'){
            // Update insert ref_nanapet in dolibarr
            $this->db->begin();
            $split_id = explode(",",$id);
            $sql = "UPDATE " . MAIN_DB_PREFIX . "product";
            $sql.= " SET ref_nanapet = " .$split_id[0];
            $sql.= " WHERE rowid = " .$split_id[1];
        ...
        if ($result >= 0) {
            ...
            $sql.= ", seuil_max_stock_alerte = " . ((isset($this->seuil_max_stock_alerte) && $this->seuil_max_stock_alerte != '') ? "'" . $this->seuil_max_stock_alerte . "'" : "null");
            ...
            /* NanaPet*/
            $sql.= ", manufacturer = '" . $this->db->escape($this->manufacturer) . "'";
            $sql.= ", price_nanapet = " . $this->db->escape(price2num($this->sellingPriceNanaPet));
            $sql.= ", price_discount_nanapet = " . $this->db->escape(price2num($this->discountPriceNanaPet));            
            $sql.= ", price_ttc = " . $this->db->escape(price2num($this->price));
            $sql.= ", price = " . $this->db->escape(price2num($this->price));
            $sql.= ", species = '" . $this->db->escape(price2num($this->species)) . "'";
            $sql.= ", status_product = '" . $this->db->escape(price2num($this->status_product)) . "'";
            $sql.= ", keywords = '" . $this->db->escape(price2num($this->keywords)) . "'";
            /* End NanaPet*/
            ...
            if ($resql) {
                ...
                if (!$error) {                        
                    if($flagInsert == false) {
                        if ($this->updateProductNanapet($id)) {
                            $this->db->commit();
                            return 1;
                        } else {
                            $this->error = 'Fail To Update Database NanaPet';
                            $error++;
                            $this->db->rollback();
                            return -$error;
                        }
    ...
    function delete($id = 0) {
        ...
        /* DB NanaPet */
        // Get ref_nanapet of product on Dolibarr
        $temp_ref_nanapet = 0;
        $sql = "SELECT ref_nanapet, label";
        $sql.= " FROM " . MAIN_DB_PREFIX . "product";
        if ($id) {
            $sql.= " WHERE rowid = " . $id;
        }
        ...
        if ($resql) {
            ...
            $temp_ref_nanapet = $obj->ref_nanapet;
        }
        ...
        if (empty($objectisused)) {
            ...
            if (!$error) {                
                if($this->deleteProductNanapet($temp_ref_nanapet)) {
                    $this->db->commit();
                    return 1;
                } else {
                    $error++;
                    $this->error = 'Fail To Delete Database NanaPet';
                    $this->db->rollback();
                    return -$error;
                }                
            } else {
    ...
    function fetch($id = '', $ref = '', $ref_ext = '', $ignore_expression = 0) {
        ...
        $sql.= " tobuy, fk_product_type, duration, seuil_stock_alerte, seuil_max_stock_alerte, canvas,";
        $sql.= " weight, weight_units, length, length_units, surface, surface_units, volume, volume_units, barcode, fk_barcode_type, finished,";
        $sql.= " accountancy_code_buy, accountancy_code_sell, stock, pmp,";
        $sql.= " datec, tms, import_key, entity, desiredstock, tobatch, fk_unit";
        $sql.= " , fk_price_expression";
        $sql.= " , manufacturer, price_nanapet, price_discount_nanapet, species, status_product, keywords";
        $sql.= " FROM " . MAIN_DB_PREFIX . "product";
        if ($id) {
        ...
        if ($resql) {
            if ($this->db->num_rows($resql) > 0) {
                ...
                $this->sellingPriceNanaPet = price2num($obj->price_nanapet);
                $this->discountPriceNanaPet = price2num($obj->price_discount_nanapet);
                $this->status_product = $obj->status_product;
                ...
                $this->seuil_max_stock_alerte = $obj->seuil_max_stock_alerte;
     ...
     * 	@return     int     				<0 if KO, >0 if OK
     */
    function correct_stock($user, $id_entrepot, $nbpiece
                , $movement, $label = '', $price = 0
                , $inventorycode = '', $dateExpired) {
        if ($id_entrepot) {
            $this->db->begin();
            ...
            $result = $movementstock->_create($user, $this->id, $id_entrepot, 
                    $op[$movement], $movement, $price, $label, 
                    $inventorycode, "", "", "", "", false, $dateExpired);
            if ($result >= 0) {
                $this->db->commit();
                return 1;
            } else if ($result == -10) {
    ...
    public function insertProductNanapet() {
        /* DB NanaPet */
        if($this->dbNanapet != null){            
            $products_name = $this->dbNanapet->escape($this->label);
            $manufacturer =  $this->dbNanapet->escape($this->manufacturer);
            $species = $this->dbNanapet->escape($this->species);
            $products_price = formatMoney(price2num($this->sellingPriceNanaPet));
            $product_encourage = formatMoney(price2num($this->discountPriceNanaPet));
            $products_status = "0";
            $products_date_added = time();
            $language ="vi";
            $status ="";
            if ($this->status_product == 0) {
                $status = "Co Hang";
            } else if ($this->status_product == 3) {
                $status = "Tam Thoi Het Hang";
            } else if ($this->status_product == -3) {
                $status = "Sap Co Hang";
            } else if ($this->status_product == -6) {
                $status = "Dat Hang Truoc";
            }
            $this->dbNanapet->begin();
            $sql = "INSERT INTO products(products_name
                                        ,manufacturer
                                        ,species
                                        ,products_price
                                        ,product_encourage
            ...
            if ($result) {
                $idNanaPet = $this->dbNanapet->last_insert_id_nanapet();                
                if ($this->update($idNanaPet.",".$this->id_dolibarr, "", false, 'updateNanapet') > 0) {
    ...
    public function updateProductNanapet($id) {
        /* DB NanaPet */
        if($this->dbNanapet != null) {
            // Get ref_nanapet on Dolibarr
            $temp_ref_nanapet = "";
            $sql = "SELECT ref_nanapet, label";
            $sql.= " FROM " . MAIN_DB_PREFIX . "product";
            if ($id) {
            ...
            if ($temp_ref_nanapet != "") {
                ...
                $this->dbNanapet->begin();
                $sql = "UPDATE products SET products_key='".$products_key
                                        ."',products_name='".$products_name
                                        ."',manufacturer='".$manufacturer
                                        ."',species='".$species
                                        ."',products_price='".$products_price
                                        ."',product_encourage='".$product_encourage
                                        ."',status='".$status
                                        ."',modified_date=".time()
                                        ." WHERE products_id = ".$temp_ref_nanapet;
                $result = $this->dbNanapet->query($sql);
                if($result) {
                    $this->dbNanapet->commit();
                    return true;
    ...
    public function deleteProductNanapet($id) {
        if($this->dbNanapet != null){
            if ($id != 0) {
                $this->dbNanapet->begin();
    ...
    public function syncProductsNanapet($user) {
        if($this->dbNanapet != null){
            $sql = "SELECT products_id
                        ...
                        ,product_encourage                                                                        
                        ,status
                    FROM products ORDER BY products_id ASC";
            $resql = $this->dbNanapet->query($sql);
            
            if ($resql) {
                if ($this->dbNanapet->num_rows($resql) > 0) {
                    while ($obj = $this->dbNanapet->fetch_object($resql)) {
                        $dolibarrobject = new Product($this->db, $this->db_nanapet);
                        $dolibarrobject->ref_nanapet = $this->dbNanapet->escape($obj->products_id);
                        $dolibarrobject->ref = "SKU_" . $obj->products_id;
                        $dolibarrobject->label = $this->dbNanapet->escape($obj->products_name);
                        $dolibarrobject->manufacturer = $this->dbNanapet->escape($obj->manufacturer);
                        $dolibarrobject->species = $this->dbNanapet->escape($obj->species);                        
                        $dolibarrobject->sellingPriceNanaPet = $this->dbNanapet->escape(str_replace(".","",$obj->products_price));
                        $dolibarrobject->discountPriceNanaPet = $this->dbNanapet->escape(str_replace(".","",$obj->product_encourage));
                        if(empty($dolibarrobject->sellingPriceNanaPet)) {
                            $dolibarrobject->sellingPriceNanaPet = 0;
                        }
                        if(empty($dolibarrobject->discountPriceNanaPet)) {
                            $dolibarrobject->discountPriceNanaPet = 0;
                        }
                        
                        if ($dolibarrobject->discountPriceNanaPet > 0) {
                            $dolibarrobject->price = $dolibarrobject->discountPriceNanaPet;
                        } else {
                            $dolibarrobject->price = $dolibarrobject->sellingPriceNanaPet;
                        }
                        
                        if ($obj->status == "Co Hang") {
                            $dolibarrobject->status_product = 0;
                        } else if ($obj->status == "Tam Thoi Het Hang") {
                            $dolibarrobject->status_product = 3;
                        } else if ($obj->status == "Sap Co Hang") {
                            $dolibarrobject->status_product = -3;
                        } else if ($obj->status == "Dat Hang Truoc") {
                            $dolibarrobject->status_product = -6;
                        }                        
                        $dolibarrobject->status_buy = 1;
                                                
                        // rowid Dolibarr products
                        $temp_products_id = $this->checkProductOnDolibarr($obj->products_id);
                        if ($temp_products_id > 0) {
                            if ($dolibarrobject->syncUpdateProductOnDolibarr($temp_products_id)) {
                                //do nothing
                            } else {
                                // return the rowid of product, if it fail
                                return $this->checkProductOnDolibarr($obj->products_id);
                            }
                        } else {
                            if ($dolibarrobject->create($user,0,true) > 0 ) {
                                //do nothing
                            } else {
                                // return if fail
                                return $dolibarrobject->create($user,0,true);
                            }                            
                        }
                    }
                    return 1;
                } else {
                    return -10; // NanaPet don't have data
                }
            } else {
                return -100; // NanaPet has no data to sync
            }            
        }
        return -1000; // Don't have object NanaPet
    }
    
    public function syncUpdateProductOnDolibarr($id) {
        $this->db->begin();
        $sql = "UPDATE " . MAIN_DB_PREFIX . "product";
        $sql.= " SET label = '" . $this->db->escape($this->label) . "'";
        $sql.= ", tosell = " . $this->db->escape($this->status_buy);
        $sql.= ", manufacturer = '" . $this->db->escape($this->manufacturer) . "'";
        $sql.= ", species = '" . $this->db->escape(price2num($this->species)) . "'";        
        $sql.= ", price_nanapet = " . $this->db->escape(price2num($this->sellingPriceNanaPet));
        $sql.= ", price_discount_nanapet = " . $this->db->escape(price2num($this->discountPriceNanaPet));            
    ...
    // return the rowid of product in Dolibarr OR return 0
    public function checkProductOnDolibarr($id) {
        $sql = "SELECT rowid";        
        $sql.= " FROM " . MAIN_DB_PREFIX . "product";
        $sql.= " WHERE ref_nanapet = " . $this->db->escape($id);
        $resql = $this->db->query($sql);
        if ($resql) {
            if ($this->db->num_rows($resql) > 0) {
                $obj = $this->db->fetch_object($resql);
    ...
    public function deleteAllProductDollibar() {
        $sql = "SELECT rowid ";
        $sql.= " FROM " . MAIN_DB_PREFIX . "product";
        $resql = $this->db->query($sql);
        if ($resql) {
            if ($this->db->num_rows($resql) > 0) {
                while ($obj = $this->db->fetch_object($resql)) {
                    $this->delete($obj->rowid);
                }
            }
        }
    }
    
    public function checkUpdateForProduct() {
        if($this->dbNanapet != null && $this->db != null){
            $count_product_nanapet = 0;
            ...
            if($result) {
                if ($this->dbNanapet->num_rows($sql) > 0) {
                    ...
                    $count_product_nanapet = $obj->count_product;
                }                                
            }
            
            $count_product_dolibarr = 0;
            $sql = "SELECT count(*) as count_product"
                .  " FROM llx_product";
            $result_dolibarr = $this->db->query($sql);
            if($result_dolibarr) {
                if ($this->db->num_rows($sql) > 0) {
                    $obj = $this->db->fetch_object($sql);
                    $count_product_dolibarr = $obj->count_product;
                }
            }            
            if($count_product_nanapet > $count_product_dolibarr) {
                return -1;
            } else if ($count_product_nanapet < $count_product_dolibarr) {
                return 0;
            } else if ($count_product_nanapet == $count_product_dolibarr) {
                return 1;
            }
