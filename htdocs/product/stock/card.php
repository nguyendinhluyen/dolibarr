...
if ($action == 'create') {
    ...
    if ($id) {
        ...
        if ($action <> 'edit' && $action <> 're-edit') {
            ...
            print formatMoney((empty($calcproducts['value']) ? '0' : price2num($calcproducts['value'], 'MT')), 0, $langs, 0, -1, -1, $conf->currency);
            print "</td></tr>";*/
            ...
            print_liste_field_titre($langs->trans("SKU"), "", "p.ref", "&amp;id=" . $id, "", "", $sortfield, $sortorder);
            print_liste_field_titre($langs->trans("ProductName"), "", "p.label", "&amp;id=" . $id, "", "", $sortfield, $sortorder);
            ...
            if (empty($conf->global->PRODUIT_MULTIPRICES))
                print_liste_field_titre($langs->trans("SellPriceMin"), "", "p.price", "&amp;id=" . $id, "", 'align="right"', $sortfield, $sortorder);
            if (empty($conf->global->PRODUIT_MULTIPRICES))
                print_liste_field_titre($langs->trans("EstimatedStockValueSellShort"), "", "", "&amp;id=" . $id, "", 'align="right"', $sortfield, $sortorder);
            if ($user->rights->stock->mouvement->creer)
                print_liste_field_titre('');
            if ($user->rights->stock->creer)
                print_liste_field_titre('');           
            print_liste_field_titre($langs->trans(""), "", "", "&amp;id=" . $id, "", 'align="right"', $sortfield, $sortorder);
            print_liste_field_titre($langs->trans(""), "", "", "&amp;id=" . $id, "", 'align="right"', $sortfield, $sortorder);
            print "</tr>\n";
            ...
            if ($resql) {
                ...
                while ($i < $num) {
                    ...
                    // Price buy PMP
                    // print '<td align="right">' . formatMoney(price2num($objp->ppmp, 'MU')) . '</td>';                    
                    // Total PMP
                    // print '<td align="right">' . formatMoney(price2num($objp->ppmp * $objp->value, 'MT')) . '</td>';
                    ...
                    if (empty($conf->global->PRODUIT_MULTIPRICES)) {
                        ...
                        print formatMoney(price2num($pricemin, 'MU'), 0);
                        print '</td>';
                        // Total sell min
                        print '<td align="right">';
                        print formatMoney(price2num($pricemin * $objp->value, 'MT'), 0);
                        print '</td>';
                    }
                ...
                //print '<td class="liste_total" align="right">' . formatMoney(price2num($totalvalue, 'MT')) . '</td>';
                if (empty($conf->global->PRODUIT_MULTIPRICES)) {
                    print '<td class="liste_total">&nbsp;</td>';
                    print '<td class="liste_total" align="right">' . formatMoney(price2num($totalvaluesell, 'MT')) . '</td>';
                }
                print '<td class="liste_total">&nbsp;</td>';
                print '<td class="liste_total">&nbsp;</td>';
                print '</tr>';
