...
class MouvementStock extends CommonObject {
    ...
    function _create($user, $fk_product, $entrepot_id, $qty, $type
                    , $price = 0, $label = '', $inventorycode = ''
                    , $datem = '', $eatby = '', $sellby = '', $batch = ''
                    , $skip_batch = false, $dateexpired) {
        global $conf, $langs;
        ...
        if (!empty($conf->productbatch->enabled) && $product->hasbatch() && !$skip_batch) {
            ...
            $sql = "SELECT pb.rowid, pb.batch, pb.eatby, pb.sellby FROM " 
                    . MAIN_DB_PREFIX . "product_batch as pb, " 
                    . MAIN_DB_PREFIX . "product_stock as ps";
            $sql.= " WHERE pb.fk_product_stock = ps.rowid AND ps.fk_product = " 
                    . $fk_product 
        ...
        if ($movestock && $entrepot_id > 0) { // Change stock for current product, change for subproduct is done after
            ...
            if (!$error) {
                ...
                $sql.= " WHERE fk_entrepot = " . $entrepot_id 
                        ...
                        . " AND dateexpired = '" . $this->db->idateformatYMD($dateexpired)."'"; // This is a unique key
                dol_syslog(get_class($this) . "::_create", LOG_DEBUG);
                $resql = $this->db->query($sql);
                if ($resql) {
                    $obj = $this->db->fetch_object($resql);
            ...
            if (!$error) {
                if ($num > 0) {
                    ...
                    $sql.= " WHERE fk_entrepot = " . $entrepot_id 
                            . " AND fk_product = " . $fk_product
                            . " AND dateexpired = '" . $this->db->idateformatYMD($dateexpired)."'";
                } else {
                    if ($qty > 0) {
                        $sql = "INSERT INTO " . MAIN_DB_PREFIX . "product_stock";
                        //$sql.= " (pmp, reel, fk_entrepot, fk_product) VALUES ";
                        //$sql.= " (".$newpmpwarehouse.", ".$qty.", ".$entrepot_id.", ".$fk_product.")";
                        $sql.= " (reel, fk_entrepot, fk_product, dateexpired) VALUES ";
                        $sql.= " (" . $qty . ", " . $entrepot_id . ", " . $fk_product ;
                        $sql.= ", " . (dateexpired ? "'" . $this->db->idate($dateexpired) . "'" : "null");
                        $sql.= ")";
                    } else {
                        return -100;
                    }
            ...
            if (!$error) {
                $sql = "UPDATE " . MAIN_DB_PREFIX . "product SET pmp = " . $newpmp 
                        . ", stock = " . $this->db->ifsql("stock IS NULL", 0, "stock") 
                        . " + " . $qty;
