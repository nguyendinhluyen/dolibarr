...
$stockmaxlimit = GETPOST('stockmaxlimit');
...
// Set stock limit
// Nguong sp thap nhat phai co trong kho
if ($action == 'setstocklimit') {
    $product = new Product($db);
    $result = $product->fetch($id);
    if($stocklimit >= $product->seuil_max_stock_alerte) {        
        setEventMessage($langs->trans("ErrorMinLimit"), 'errors');
    } else {
        $product->seuil_stock_alerte = $stocklimit;
        $result = $product->update($product->id, $user, 0, 'update',true);
        if ($result < 0) {
...
// Nguong sp cao nhat phai co trong kho
if ($action == 'setstockmaxlimit') {
    $product = new Product($db);
    $result = $product->fetch($id);        
    if($stockmaxlimit <= $product->seuil_stock_alerte) {
        setEventMessage($langs->trans("ErrorMaxLimit"), 'errors');
    } else  {
        $product->seuil_max_stock_alerte = $stockmaxlimit;
        $result = $product->update($product->id, $user, 0, 'update',true);
        if ($result < 0) {
            setEventMessage($product->error, 'errors');
        }
...
if ($action == "correct_stock" && !$cancel) {   
    ...
    if (!GETPOST("liv_")) {
        setEventMessage($langs->trans("ErrorFieldRequired", $langs->transnoentitiesnoconv("DateExpried")), 'errors');
        $error++;
        $action = 'correction';
    }
    ...
    // NanaPet - Check number of products
    $nbpiece = GETPOST("nbpiece");
    if (!is_numeric($nbpiece)) {                
        setEventMessage($langs->trans("ErrorNumberOfProducts"), 'errors');
        $error++;
        $action = 'correction';
    } else if (is_numeric($nbpiece)){
        if ($nbpiece <= 0) {
            setEventMessage($langs->trans("ErrorNumberOfProducts"), 'errors');
            $error++;
            $action = 'correction';
        ...
        if (is_numeric(GETPOST("nbpiece")) && $id) {
            ...
            if ($product->hasbatch()) {
                $result = $product->correct_stock_batch(
                        ...
                        $priceunit, $d_eatby, $d_sellby, GETPOST('batch_number'), GETPOST('inventorycode')
                );  // We do not change value of stock for a correction
            } else {
                // NanaPet 
                $dateExpired = dol_mktime(12, 0, 0, GETPOST('liv_month'), 
                        GETPOST('liv_day'), 
                        GETPOST('liv_year'));
                $result = $product->correct_stock(
                        $user, GETPOST("id_entrepot"), 
                        GETPOST("nbpiece"), GETPOST("mouvement"), GETPOST("label"), 
                        $priceunit, GETPOST('inventorycode'), $dateExpired);  // We do not change value of stock for a correction
            }
            ...
            if ($result > 0) {
                ...
                setEventMessages($langs->trans("OverLoadProductStore"), null, 'errors');                
                $action = 'correction';                
            } else {
                setEventMessage($product->error, 'errors');
                $action = 'correction';
    ...
    if (!$error) {
        if ($id) {
            ...
            if ($product->hasbatch()) {
                ...
                // NanaPet - Date expried
                $dateExpired = dol_mktime(12, 0, 0, GETPOST('liv_tranfer_month')
                            , GETPOST('liv_tranfer_day')
                            , GETPOST('liv_tranfer_year'));
                // Remove stock
                $result1 = $product->correct_stock(
                        $user, GETPOST("id_entrepot_source")
                        , GETPOST("nbpiece"), 1, GETPOST("label")
                        , $pricesrc, GETPOST('inventorycode'), $dateExpired);
                if ($result1 > 0) {
                    // Add stock
                    $result2 = $product->correct_stock(
                            $user, GETPOST("id_entrepot_destination")
                            , GETPOST("nbpiece"), 0, GETPOST("label")
                            , $pricedest, GETPOST('inventorycode'), $dateExpired);
                }
            }
            if (!$error && $result1 >= 0 && $result2 >= 0) {
                ...
                setEventMessages($langs->trans("OverLoadProductTransfer"), null, 'errors');
                $db->rollback();
                $action = 'transfert';
            } else {
...
if ($id > 0 || $ref) {
    ...
    if ($result > 0) {
        ...
        // Ref
        print '<tr>';
        print '<td width="30%">' . $langs->trans("Sku") . '</td><td>';
        print $form->showrefnav($product, 'ref', '', 1, 'ref');
        print '</td>';
        print '</tr>';

        // Label
        print '<tr><td>' . $langs->trans("ProductName") . '</td><td>' . $product->label . '</td>';
        print '</tr>';
        ...
        print '<td>';
        if ($product->pmp > 0) {
            print formatMoney(price2num($product->pmp));
        }
        print '</td>';
        print '</tr>';
        ...
        if (empty($conf->global->PRODUIT_MULTIPRICES)) {
            ...
            print formatMoney(price2num($object->price));
            ...
            if ($object->price_base_type == 'TTC') {
                print formatMoney(price2num($object->price_min_ttc)) . ' ' . $langs->trans($object->price_base_type);
            } else {
                print formatMoney(price2num($object->price_min)) . ' ' . $langs->trans($object->price_base_type);
            }
            print '</td></tr>';*/
        } else {
            // Price
        ...
        // Stock max alert threshold
        print '<tr><td>'. $form->editfieldkey("StockMaxLimit", 'stockmaxlimit', 
                        $product->seuil_max_stock_alerte, 
                        $product, 
                        $user->rights->produit->creer) 
                        .'</td><td colspan="2">';
        print $form->editfieldval("StockMaxLimit", 
                    'stockmaxlimit', 
                    $product->seuil_max_stock_alerte, 
                    $product, 
                    $user->rights->produit->creer);
        ...
        print '<tr><td>' . $form->editfieldkey("StockLimit", 'stocklimit', 
                        $product->seuil_stock_alerte, 
                        $product, 
                        $user->rights->produit->creer) . '</td><td colspan="2">';
        print $form->editfieldval("StockLimit", 'stocklimit', 
                        $product->seuil_stock_alerte, 
                        $product, 
                        $user->rights->produit->creer);
        print '</td></tr>';
        ...
        } else if($product->stock_reel > $product->seuil_max_stock_alerte) {
            print ' ' . img_warning($langs->trans("StockMoreThanLimit"));
        }
        print '</td>';
        print '</tr>';
        ...
        if ($product->stock_theorique < $product->seuil_stock_alerte) {
            print ' ' . img_warning($langs->trans("StockLowerThanLimit"));
        } else if($product->stock_theorique > $product->seuil_max_stock_alerte) {
            print ' ' . img_warning($langs->trans("StockMoreThanLimit"));
        }
...
print '<td align="right">' . $langs->trans("DateExpired") . '</td>';
print '<td align="right">' . $langs->trans("") . '</td>';
print '<td align="right">' . $langs->trans("") . '</td>';
...
$sql = "SELECT e.rowid, e.label, e.lieu, ps.reel, ps.pmp, ps.rowid as product_stock_id, ps.dateexpired";
...
if ($resql) {
    ...
    while ($i < $num) {
        ...
        // NanaPet - Date expired
        print '<td align="right">' . substr($obj->dateexpired, 5, 2)."-"
                .substr($obj->dateexpired, 8, 2)."-"                
                .substr($obj->dateexpired, 0, 4)  . '</td>';
        print '<td align="right"/>'; 
        print '<td align="right"/>'; 
        // PMP
        // print '<td align="right">' . (price2num($product->pmp) ? formatMoney(price2num($product->pmp, 'MU')) : '') . '</td>';
        // Value purchase
        // print '<td align="right">' . (price2num($product->pmp) ? formatMoney(price2num($product->pmp * $obj->reel, 'MT')) : '') . '</td>';
        // Sell price
        print '<td align="right">';
        if (empty($conf->global->PRODUIT_MULTI_PRICES)) {
            print formatMoney(price2num($product->price, 'MU'));
        }
        else {
            print $langs->trans("Variable");
        }
        ...
        print '<td align="right">';
        if (empty($conf->global->PRODUIT_MULTI_PRICES)) {
            print formatMoney(price2num($product->price * $obj->reel, 'MT')) . '</td>';
        }
        else {
            print $langs->trans("Variable");
        }
...
print '<td align="right">' . $langs->trans("") . '</td>';
//print ($totalwithpmp ? formatMoney(price2num($totalvalue / $totalwithpmp, 'MU')) : '&nbsp;'); // This value may have rounding errors
print '</td>';
// Value purchase
print '<td class="liste_total" align="right">';
// print $totalvalue ? formatMoney(price2num($totalvalue, 'MT')) : '&nbsp;';
print '</td>';
print '<td class="liste_total" align="right">';
//if (empty($conf->global->PRODUIT_MULTI_PRICES)) {
//    print ($total ? formatMoney(price2num($totalvaluesell / $total, 1)) : '&nbsp;');
//}
//else {
//    print $langs->trans("Variable");
//}
...
print '<td class="liste_total" align="right">';
if (empty($conf->global->PRODUIT_MULTI_PRICES)) {
    print formatMoney(price2num($totalvaluesell, 'MT'));
}
else {
    print $langs->trans("Variable");
}
