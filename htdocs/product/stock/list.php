...
if ($search_ref)
    $sql.= natural_search("e.label", $search_ref);   // ref
if ($search_label)
    $sql.= natural_search("e.lieu", $search_label);  // label
    ...
    if ($num) {
        ...
        while ($i < min($num, $limit)) {
            ...
            if (price2num($objp->estimatedvalue, 'MT')) {
                print formatMoney(price2num($objp->estimatedvalue, 'MT'));
            }
            else
                print '';
            print '</td>';
            ...
            if (empty($conf->global->PRODUIT_MULTIPRICES)) {
                print formatMoney(price2num($objp->sellvalue, 'MT'));
            }
            else {
        ...
        print '<td align="right">' . formatMoney(price2num($total, 'MT'), 0, $langs, 0, 0, -1, $conf->currency) . '</td>';
        print '<td align="right">';
        if (empty($conf->global->PRODUIT_MULTIPRICES)) {
            print formatMoney(price2num($totalsell, 'MT'));
        }
        else {
            $htmltext = $langs->trans("OptionMULTIPRICESIsOn");
            print $form->textwithtooltip($langs->trans("Variable"), $htmltext);
