            ...
            if ($resql && $db->num_rows($resql) > 0) {
                ...
                foreach ($supplier['lines'] as $line) {
                    $result = $order->addline(
                            $line->desc, $line->subprice, $line->qty, $line->tva_tx, $line->localtax1_tx, $line->localtax2_tx, $line->fk_product, 0, $line->ref_fourn, $line->remise_percent, 'HT', 0, $line->info_bits
                    );
                }
                if ($result < 0) {
                    $fail++;
...
if ($sref || $snom || $sall || $salert || GETPOST('search', 'alpha')) {
    ...
    $filters .= '&mode=' . $mode;
    print_barre_liste(
            $texte, $page, 'replenish.php', $filters, $sortfield, $sortorder, '', $num
    );
} else {
    $filters = '&sref=' . $sref . '&snom=' . $snom;
