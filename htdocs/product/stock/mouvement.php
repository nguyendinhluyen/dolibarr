...
if ($cancel)
    $action = ''; // Protection to avoid action for all cancel buttons
    ...
    if (!$error) {
        if ($product->hasbatch()) {
            ...
            $result = $product->correct_stock_batch(
                    $user, $id, GETPOST("nbpiece", 'int'), GETPOST("mouvement"), GETPOST("label", 'san_alpha'), GETPOST('unitprice'), $eatby, $sellby, $batch, GETPOST('inventorycode')
            );  // We do not change value of stock for a correction
        } else {
            $result = $product->correct_stock(
                    $user, $id, GETPOST("nbpiece", 'int'), GETPOST("mouvement"), GETPOST("label", 'san_alpha'), GETPOST('unitprice'), GETPOST('inventorycode')
            );  // We do not change value of stock for a correction
        }
...
if (!empty($search_batch))
    $sql.= " AND m.batch LIKE '%" . $db->escape($search_batch) . "%'";
...
if ($resql) {
    ...
    if (!empty($sref))
        $param.='&sref=' . urlencode($sref); // FIXME $sref is not defined
    if (!empty($snom))
        $param.='&snom=' . urlencode($snom); // FIXME $snom is not defined
    if ($search_user)
    ...
    print_liste_field_titre($langs->trans("ProductName"), $_SERVER["PHP_SELF"], "p.ref", "", $param, "", $sortfield, $sortorder);
