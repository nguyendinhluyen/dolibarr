...
if ($id || $ref) {
    ...
    if ($result) {
        if ($action <> 'edit' && $action <> 're-edit') {
            ...
            // Reference
            print '<tr>';
            print '<td width="15%">' . $langs->trans("Sku") . '</td><td colspan="2">';
            print $form->showrefnav($product, 'ref', '', 1, 'ref');
            print '</td>';
            print '</tr>';

            // Label
            print '<tr><td>' . $langs->trans("ProductName") . '</td><td colspan="2">' . $product->label . '</td></tr>';
            ...
            if ($user->rights->fournisseur->lire) {
                ...
                if (count($product_fourn_list) > 0) {
                    ...
                    foreach ($product_fourn_list as $productfourn) {
                        ...
                        print '<td align="right">';
                        print $productfourn->fourn_price ? formatMoney(price2num($productfourn->fourn_price)) : "";
                        print '</td>';
                        ...
                        if ($conf->global->PRODUCT_CHARGES) {
                            if (!empty($conf->margin->enabled)) {
                                print '<td align="right">';
                                print $productfourn->fourn_charges ? formatMoney(price2num($productfourn->fourn_charges)) : "";
                                print '</td>';
                            }
                        }
                        ...
                        // Unit price
                        print '<td align="right">';
                        print formatMoney(price2num($productfourn->fourn_unitprice));
