...
// Label
print '<tr><td>' . $langs->trans("ProductName") . '</td><td>' . $object->label . '</td>';
...
if ($result) {
    ...
    if ($num > 0) {
        if (!empty($conf->global->PRODUIT_CUSTOMER_PRICES)) {
            print_fiche_titre($langs->trans("DefaultPrice"), '', '');
        }
        ...
        while ($i < $num) {
            ...
            print '<td align="right">' . formatMoney(price2num($objp->price)) . '</td>';
            // User
