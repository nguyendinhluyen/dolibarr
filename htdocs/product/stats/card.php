...
if (!empty($id) || !empty($ref)) {
    ...
    if ($result) {
        ...
        // Reference
        print '<tr>';
        print '<td width="30%">' . $langs->trans("Sku") . '</td><td colspan="3">';
        print $form->showrefnav($object, 'ref', '', 1, 'ref');
        print '</td>';
        print '</tr>';

        // Label
        print '<tr><td>' . $langs->trans("ProductName") . '</td><td colspan="3">' . $object->label . '</td></tr>';
        ...
        if ($mode == 'bynumber')
            print '<a href="' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&mode=byunit">';
        else
        ...
        if ($mode == 'byunit')
            print '<a href="' . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&mode=bynumber">';
        else
            print img_picto('', 'tick') . ' ';
        ...
        if (!$error) {
            ...
            if (!$mesg) {
                foreach ($graphfiles as $key => $val) {
                    ...
                    if ($key == 'propal')
                        $graph_data = $object->get_nb_propal($socid, $mode);
                    if ($key == 'orders')
                        $graph_data = $object->get_nb_order($socid, $mode);
                    if ($key == 'invoices')
                        $graph_data = $object->get_nb_vente($socid, $mode);
                    if ($key == 'invoicessuppliers')
                        $graph_data = $object->get_nb_achat($socid, $mode);
                    if ($key == 'orderssuppliers')
                        $graph_data = $object->get_nb_ordersupplier($socid, $mode);
        ...
        foreach ($graphfiles as $key => $val) {
            ...
            if ($graphfiles[$key]['output'] && !$px->isGraphKo()) {
                if (file_exists($dir . "/" . $graphfiles[$key]['file']) && filemtime($dir . "/" . $graphfiles[$key]['file']))
                    ...
                    print '<td>' . $langs->trans("GeneratedOn", dol_print_date(dol_now()), "dayhour") . '</td>';
            }
            else {
