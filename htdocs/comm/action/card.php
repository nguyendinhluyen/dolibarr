        ...
        if ($idaction > 0) {
            if (!$object->error) {
                ...
                if ($user->id != $object->ownerid)
                    $moreparam = "usertodo=-1"; // We force to remove filter so created record is visible when going back to per user view.
...
if ($id > 0) {
    ...
    if ($action == 'edit') {
        ...
        if (GETPOST("afaire") == 1)
            $form->select_date($datep ? $datep : $object->datep, 'ap', 1, 1, 0, "action", 1, 1, 0, 0, 'fulldaystart');
        else if (GETPOST("afaire") == 2)
            $form->select_date($datep ? $datep : $object->datep, 'ap', 1, 1, 1, "action", 1, 1, 0, 0, 'fulldaystart');
        else
            $form->select_date($datep ? $datep : $object->datep, 'ap', 1, 1, 1, "action", 1, 1, 0, 0, 'fulldaystart');
        print '</td></tr>';
        // Date end
        ...
        if (GETPOST("afaire") == 1)
            $form->select_date($datef ? $datef : $object->datef, 'p2', 1, 1, 1, "action", 1, 1, 0, 0, 'fulldayend');
        else if (GETPOST("afaire") == 2)
            $form->select_date($datef ? $datef : $object->datef, 'p2', 1, 1, 1, "action", 1, 1, 0, 0, 'fulldayend');
        ...
        if (empty($donotclearsession)) {
            if ($object->userownerid > 0)
                $listofuserid[$object->userownerid] = array('id' => $object->userownerid, 'transparency' => $object->userassigned[$user->id]['transparency'], 'answer_status' => $object->userassigned[$user->id]['answer_status'], 'mandatory' => $object->userassigned[$user->id]['mandatory']); // Owner first
