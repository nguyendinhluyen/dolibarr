...
//				$this->durationp			= $this->durationp;					// deprecated
                ...
                if (!is_object($this->author))
                    $this->author = new stdClass(); // For avoid warning
                ...
                if (!is_object($this->usermod))
                    $this->usermod = new stdClass(); // For avoid warning
...
//if ($this->date  && $this->dateend) $this->durationa=($this->dateend - $this->date);
        ...
        if (!empty($this->fk_element))
            $sql.= ", fk_element=" . ($this->fk_element ? $this->fk_element : "null");
        if (!empty($this->elementtype))
            $sql.= ", elementtype=" . ($this->elementtype ? "'" . $this->elementtype . "'" : "null");
        $sql.= " WHERE id=" . $this->id;
    ...
    function getNomUrl($withpicto = 0, $maxlength = 0, $classname = '', $option = '', $overwritepicto = 0) {
        ...
        if ($option == 'birthday')
            $link = '<a class="' . $classname . ' classfortooltip" href="' . DOL_URL_ROOT . '/contact/perso.php?id=' . $this->id . $linkclose;
        else
            $link = '<a class="' . $classname . ' classfortooltip" href="' . DOL_URL_ROOT . '/comm/action/card.php?id=' . $this->id . $linkclose;
        $linkend = '</a>';
        ...
        if ($withpicto == 2) {
            ...
            if (!empty($conf->global->AGENDA_USE_EVENT_TYPE))
                $libelle = $langs->transnoentities("Action" . $this->type_code);
            $libelleshort = '';
        }
        else {
            ...
            if (!empty($conf->global->AGENDA_USE_EVENT_TYPE) && empty($libelle))
                $libelle = ($langs->transnoentities("Action" . $this->type_code) != "Action" . $this->type_code) ? $langs->transnoentities("Action" . $this->type_code) : $this->type_label;
            if ($maxlength < 0)
