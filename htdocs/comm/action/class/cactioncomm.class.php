...
class CActionComm {
    ...
    function liste_array($active = '', $idorcode = 'id', $excludetype = '', $onlyautoornot = 0) {
        ...
        if (!empty($excludetype))
            $sql.=($active != '' ? " AND" : " WHERE") . " type <> '" . $excludetype . "'";
        ...
        if ($resql) {
            ...
            if ($nump) {
                ...
                while ($i < $nump) {
                    ...
                    if ($qualified && $onlyautoornot && preg_match('/^system/', $obj->type) && !preg_match('/^AC_OTH/', $obj->code))
                    ...
                    if ($qualified) {
                        $code = $obj->code;
                        if ($onlyautoornot && $code == 'AC_OTH')
                            $code = 'AC_MANUAL';
                        if ($onlyautoornot && $code == 'AC_OTH_AUTO')
                            $code = 'AC_AUTO';
                        ...
                        if ($onlyautoornot && preg_match('/^module/', $obj->type) && $obj->module)
