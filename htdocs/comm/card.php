...
$object = new Client($db,$db_nanapet);
...
if ($id > 0) {
    ...
    if ($user->rights->societe->creer && !$user->societe_id > 0) {
        print '<a href="' . DOL_URL_ROOT . '/comm/remise.php?id=' . $object->id . '">' 
                . img_edit($langs->trans("Modify")) . '</a>';
    }
    print '</td>';*/    
    /*print '</tr></table>';
    print '</td><td colspan="3">' 
        . ($object->remise_percent ? '<a href="' . DOL_URL_ROOT . '/comm/remise.php?id=' 
        . $object->id . '">' . $object->remise_percent . '%</a>' : $langs->trans("DiscountNone")) . '</td>';*/
    
    $array_discount = $object->getDiscountOfCustomer($object->email);
    if($array_discount[0] > 0) {
        $object->remise_percent = $array_discount[0]. "% - ". $array_discount[1];
    }    
    print '</tr></table>';
    print '</td><td colspan="3">'
        . ($object->remise_percent ? '<b style="color:red">' 
        . $object->remise_percent . '</b>' : $langs->trans("DiscountNone")) . '</td>';
    print '</tr>';
    ...
    if (!empty($conf->facture->enabled) && $user->rights->facture->lire) {
        ...
        if ($resql) {
            ...
            while ($i < $num && $i < $MAXLIST) {
                ...
                print '<td align="right" width="120">' . formatMoney(price2num($objp->total_ht)) . '</td>';
