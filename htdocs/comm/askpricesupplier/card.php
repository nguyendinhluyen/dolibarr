...
if (empty($reshook)) {
    ...
    // Validation
            ...
            ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->askpricesupplier->creer)) || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->askpricesupplier->validate)))
    ) {
...
if ($action == 'create') {
    ...
    if (!empty($conf->global->ASKPRICESUPPLIER_CLONE_ON_CREATE_PAGE) || !empty($conf->global->PRODUCT_SHOW_WHEN_CREATE))
        print '<br><table>';
    if (!empty($conf->global->ASKPRICESUPPLIER_CLONE_ON_CREATE_PAGE)) {
