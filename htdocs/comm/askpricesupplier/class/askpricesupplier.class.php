...
class AskPriceSupplier extends CommonObject {
            ...
            if ($this->id) {
                ...
                if (!$error) {
                    ...
                    for ($i = 0; $i < $num; $i++) {
                        ...
                        $result = $this->addline(
                                $this->lines[$i]->desc, $this->lines[$i]->subprice, $this->lines[$i]->qty, $this->lines[$i]->tva_tx, $this->lines[$i]->localtax1_tx, $this->lines[$i]->localtax2_tx, $this->lines[$i]->fk_product, $this->lines[$i]->remise_percent, 'HT', 0, 0, $this->lines[$i]->product_type, $this->lines[$i]->rang, $this->lines[$i]->special_code, $fk_parent_line, $this->lines[$i]->fk_fournprice, $this->lines[$i]->pa_ht, $this->lines[$i]->label, $this->lines[$i]->array_options, $this->lines[$i]->ref_fourn
                        );
    ...
    function valid($user, $notrigger = 0) {
        ...
        if ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->askpricesupplier->creer)) || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->askpricesupplier->validate))) {
            $this->db->begin();
    ...
    function load_board($user, $mode) {
        ...
        if ($resql) {
            if ($mode == 'opened')
                $delay_warning = $conf->askpricesupplier->cloture->warning_delay;
            if ($mode == 'signed')
                $delay_warning = $conf->askpricesupplier->facturation->warning_delay;
