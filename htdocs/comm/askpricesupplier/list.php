...
if ($sall)
    $sql.= ' LEFT JOIN ' . MAIN_DB_PREFIX . 'askpricesupplierdet as pd ON p.rowid=pd.fk_askpricesupplier';
...
if ($result) {
    ...
    if ($search_societe)
        $param.='&search_societe=' . $search_societe;
    if ($search_user > 0)
    ...
    while ($i < min($num, $limit)) {
        ...
        if ($objp->fk_statut == 1 && $db->jdate($objp->dfv) < ($now - $conf->askpricesupplier->cloture->warning_delay))
