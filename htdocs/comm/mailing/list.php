...
if ($filteremail) {
    ...
    if ($sref)
        $sql.= " AND m.rowid = '" . $db->escape($sref) . "'";
    if ($sall)
        $sql.= " AND (m.titre like '%" . $db->escape($sall) . "%' OR m.sujet like '%" . $db->escape($sall) . "%' OR m.body like '%" . $db->escape($sall) . "%')";
    if (!$sortorder)
        $sortorder = "ASC";
    if (!$sortfield)
        $sortfield = "m.rowid";
...
if ($result) {
    ...
    if (!$filteremail)
        print_liste_field_titre($langs->trans("NbOfEMails"), $_SERVER["PHP_SELF"], "m.nbemail", $param, "", 'align="center"', $sortfield, $sortorder);
    if (!$filteremail)
        print_liste_field_titre($langs->trans("DateLastSend"), $_SERVER["PHP_SELF"], "m.date_envoi", $param, "", 'align="center"', $sortfield, $sortorder);
    else
