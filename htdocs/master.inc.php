...
$conf = new Conf();
// DB NanaPet
$conf->db->nameNanaPet = $dolibarr_main_db_name_nanapet;
...
if (!defined('NOREQUIREDB')) {
    ...
    // DB Nanapet
    $db_nanapet = getDoliDBInstance($conf->db->type, $conf->db->host, $conf->db->user, $conf->db->pass, $conf->db->nameNanaPet, $conf->db->port);
    ...
    // DB Nanapet
    if ($db_nanapet->error) {
        dol_print_error($db, "host=" . $conf->db->host . ", port=" . $conf->db->port . ", user=" . $conf->db->user . ", databasename=" . $conf->db->nameNanaPet . ", " . $db->error);
        exit;
    }
}
...
if (!empty($dolibarr_pdf_force_fpdf))
    $conf->global->MAIN_USE_FPDF = $dolibarr_pdf_force_fpdf;