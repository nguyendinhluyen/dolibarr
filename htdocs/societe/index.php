...
// Check to sync DB NanaPet
$syncDbNanapet = GETPOST('syncDbNanapet');
$object = new Societe($db, $db_nanapet);
if (!$syncDbNanapet) {
    if($object->checkUpdateForCustomer() == -1) {
        setEventMessage($langs->trans("WarningsCustomerDolibarr"), 'warnings');
    } else if ($object->checkUpdateForCustomer() == 0) {
        setEventMessage($langs->trans("WarningsCustomerNanaPet"), 'warnings');
    }
}

// Sync database
$mesg_success=$langs->trans("syncDbNanaPetSuccess");
$mesg_error=$langs->trans("syncDbNanaPetError");
if($syncDbNanapet) {
    //$object->deleteAllCustomersDollibar();
    if ($object->syncCustomerNanapet($user) > 0) {
        setEventMessage($mesg_success);
    } else {
        //setEventMessage($object->syncProductsNanapet($user), 'errors');
        setEventMessage($mesg_error, 'errors');
    }
}
...
print '<form method="post" action="' . DOL_URL_ROOT . '/societe/index.php">';
print "<table><tr>";
print '<td rowspan="' . $rowspan . '">'
        . '<input type="submit" name="syncDbNanapet" class="button" value="' 
        . $langs->trans("syncDbNanapet") . '"></td></tr>';
print "</tr></table>";
print "</form> <br>\n";
...
if ($result) {
    while ($objp = $db->fetch_object($result)) {
        $found = 0;
        if (!empty($conf->societe->enabled) && empty($conf->global->SOCIETE_DISABLE_CUSTOMERS_STATS) && ($objp->client == 1 || $objp->client == 3)) {
            $found = 1;
            $third['customer'] ++;
        }
        if (!empty($conf->societe->enabled) && empty($conf->global->SOCIETE_DISABLE_PROSPECTS_STATS) && ($objp->client == 2 || $objp->client == 3)) {
            $found = 1;
            $third['prospect'] ++;
        }
        if (!empty($conf->fournisseur->enabled) && empty($conf->global->SOCIETE_DISABLE_SUPPLIERS_STATS) && $objp->fournisseur) {
            $found = 1;
            $third['supplier'] ++;
        }
...
if (!empty($conf->use_javascript_ajax) && ((round($third['prospect']) ? 1 : 0) + (round($third['customer']) ? 1 : 0) + (round($third['supplier']) ? 1 : 0) + (round($third['other']) ? 1 : 0) >= 2)) {
    ...
    if (!empty($conf->societe->enabled) && empty($conf->global->SOCIETE_DISABLE_PROSPECTS_STATS))
        $dataseries[] = array('label' => $langs->trans("Prospects"), 'data' => round($third['prospect']));
    if (!empty($conf->societe->enabled) && empty($conf->global->SOCIETE_DISABLE_CUSTOMERS_STATS))
        $dataseries[] = array('label' => $langs->trans("Customers"), 'data' => round($third['customer']));
    if (!empty($conf->fournisseur->enabled) && empty($conf->global->SOCIETE_DISABLE_SUPPLIERS_STATS))
        $dataseries[] = array('label' => $langs->trans("Suppliers"), 'data' => round($third['supplier']));
    if (!empty($conf->societe->enabled))
        $dataseries[] = array('label' => $langs->trans("Others"), 'data' => round($third['other']));
