...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
    ...
    if ($action == 'create') {
        ...
        if (!empty($conf->global->MAIN_THIRPARTY_CREATION_INDIVIDUAL) && !isset($_GET['private']) && !isset($_POST['private']))
            $private = 1;
        if (empty($private))
            $private = 0;*/
        ...
        if (GETPOST("type") != 'f' && empty($conf->global->THIRDPARTY_NOTCUSTOMERPROSPECT_BY_DEFAULT)) {
            $object->client = 3;
        }
        if (GETPOST("type") == 'c') {
            $object->client = 1;
        ...
        if (!empty($conf->fournisseur->enabled) && (GETPOST("type") == 'f' || (GETPOST("type") == '' && empty($conf->global->THIRDPARTY_NOTSUPPLIER_BY_DEFAULT)))) {
        ...
        if (!empty($conf->use_javascript_ajax)) {
            ...
            $text = '<input type="radio" id="radioprivate" class="flat" name="private" value="1"' 
                    . ($private ? ' checked' : '') . '>';
            $text.='&nbsp;';
            ...
            print '<input type="radio" id="radiocompany" class="flat" name="private"  value="0"' 
                    . ($private ? '' : ' checked') . '>';
            print '&nbsp;';            
        ...
        print '<form enctype="multipart/form-data" action="' 
            . $_SERVER["PHP_SELF"] . '" method="post" name="formsoc">';
        ...
        */
        ...
        print '<td>';
        print $form->selectyesno('assujtva_value', 0, 1); // Assujeti par defaut en creation
        print '</td>';
        ...
        */
        ...
        if (!empty($conf->incoterm->enabled)) {
            ...
            if ($user->rights->societe->creer)
                print '<a href="' . DOL_URL_ROOT . '/societe/soc.php?socid=' . $object->id . '&action=editincoterm">' . img_edit() . '</a>';
            else
                print '&nbsp;';
            print '</td></tr></table>';
            print '</td>';
