...
if ($search_phone)
    $sql .= " AND s.phone LIKE '%" . $db->escape($search_phone) . "%'";
if ($search_email)
    $sql .= " AND s.email LIKE '%" . $db->escape($search_email) . "%'";
if ($search_town)
...
if ($search_idprof5)
    $sql .= " AND s.idprof5 LIKE '%" . $db->escape($search_idprof5) . "%'";
if ($search_idprof6)
    $sql .= " AND s.idprof6 LIKE '%" . $db->escape($search_idprof6) . "%'";
// Filter on type of thirdparty
if ($search_type > 0 && in_array($search_type, array('1,3', '2,3')))
    $sql .= " AND s.client IN (" . $db->escape($search_type) . ")";
if ($search_type > 0 && in_array($search_type, array('4')))
...
if ($search_type == '0')
    $sql .= " AND s.client = 0 AND s.fournisseur = 0";
if (!empty($conf->barcode->enabled) && $sbarcode)
    $sql.= " AND s.barcode LIKE '%" . $db->escape($sbarcode) . "%'";
...
if ($resql) {
    ...
    // Lines of titles
    print '<tr class="liste_titre">';
    print_liste_field_titre($langs->trans("CustomerName"), $_SERVER["PHP_SELF"], "s.nom", "", $params, "", $sortfield, $sortorder);
    //if (!empty($conf->barcode->enabled))
    //    print_liste_field_titre($langs->trans("BarCode"), $_SERVER["PHP_SELF"], "s.barcode", $param, '', '', $sortfield, $sortorder);
    ...
    print_liste_field_titre($langs->trans("Phone"), $_SERVER["PHP_SELF"], "s.phone", "", $param, "", $sortfield, $sortorder);
    print_liste_field_titre($langs->trans("Email"), $_SERVER["PHP_SELF"], "s.email", "", $param, "", $sortfield, $sortorder);
    ...
    if (!empty($search_nom_only) && empty(c))
    ...
    // Phone
    print '<td class="liste_titre">';
    print '<input class="flat" size="10" type="text" name="search_phone" value="' . htmlspecialchars($search_phone) . '">';
    print '</td>';
    
    // Email
    print '<td class="liste_titre">';
    print '<input class="flat" size="10" type="text" name="search_email" value="' . htmlspecialchars($search_email) . '">';
    print '</td>';
    print '<td class="liste_titre"/>';
    print '<td class="liste_titre"/>';
    print '<td class="liste_titre"/>';
