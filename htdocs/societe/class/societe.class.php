...
class Societe extends CommonObject {
     ...
     */
    public function __construct($db, $dbNanapet = null) {
        $this->db = $db;
        // Object Db NanaPet
     ...
     */
    function create($user = '', $sync = false) {
        global $langs, $conf;
        ...
        if ($result >= 0) {
            $sql = "INSERT INTO " . MAIN_DB_PREFIX . "societe (nom, name_alias, entity, datec, fk_user_creat, canvas, status, ref_int, ref_ext, fk_stcomm, fk_incoterms, location_incoterms ,import_key, ref_customer)";
            ...
            if ($result) {
                ...
                // Use to update ref_customer on Dolibbar
                $this->id_dolibarr = $this->id;                
                $ret = $this->update($this->id, $user, 0, 1, 1, 'add',1,true);
                ...
                if (!$error) {
                    ...
                    if($sync == false) {                        
                        if($this->insertCustomerNanapet($user)) {
                            $this->db->commit();
                            return $this->id;
                        } else {
                            $this->db->rollback();
     ...
     *      @return int  			           			<0 if KO, >=0 if OK
     */
    function update($id, $user = '', $call_trigger = 1, $allowmodcodeclient = 0, 
                    $allowmodcodefournisseur = 0, $action = 'update', $nosyncmember = 1, $flagInsert = false) {
        ...
        if ($action == 'updateNanaPet'){
            // Update insert ref_customer in dolibarr
            $this->db->begin();
            $split_id = explode(",",$id);
            $sql = "UPDATE " . MAIN_DB_PREFIX . "societe";
            $sql.= " SET ref_customer = " .$split_id[0];
        ...
        if (empty($this->capital) || !is_numeric($this->capital))
            $this->capital = 0;
        ...
        if ($result >= 0) {
            ...
            if ($resql) {
                ...
                if (!$error) {
                    ...
                    if($flagInsert == false) {
                        if ($this->updateCustomerNanapet($id)) {
                            $this->db->commit();
                            return 1;
                        } else {
    ...
    function fetch($rowid, $ref = '', $ref_ext = '', $ref_int = '', $idprof1 = '', $idprof2 = '', $idprof3 = '', $idprof4 = '') {
        ...
        if ($rowid)
            ...
            $sql .= " WHERE s.nom = '" . $this->db->escape($ref) . "' AND s.entity IN (" . getEntity($this->element, 1) . ")";
        else if ($ref_ext)
            $sql .= " WHERE s.ref_ext = '" . $this->db->escape($ref_ext) . "' AND s.entity IN (" . getEntity($this->element, 1) . ")";
        else if ($ref_int)
            $sql .= " WHERE s.ref_int = '" . $this->db->escape($ref_int) . "' AND s.entity IN (" . getEntity($this->element, 1) . ")";
        else if ($idprof1)
            $sql .= " WHERE s.siren = '" . $this->db->escape($idprof1) . "' AND s.entity IN (" . getEntity($this->element, 1) . ")";
        else if ($idprof2)
            $sql .= " WHERE s.siret = '" . $this->db->escape($idprof2) . "' AND s.entity IN (" . getEntity($this->element, 1) . ")";
        else if ($idprof3)
            $sql .= " WHERE s.ape = '" . $this->db->escape($idprof3) . "' AND s.entity IN (" . getEntity($this->element, 1) . ")";   // TODO This request is used ? Multiple database recording provided !!
        else if ($idprof4)
            $sql .= " WHERE s.idprof4 = '" . $this->db->escape($idprof4) . "' AND s.entity IN (" . getEntity($this->element, 1) . ")";
    ...
    function delete($id, User $fuser = null, $call_trigger = 1) {
        ...
        /* DB NanaPet */
        // Get ref_customer on Dolibarr
        $temp_ref_customer = 0;
        $sql = "SELECT ref_customer";
        ...
        if ($resql) {
            ...
            $temp_ref_customer = $obj->ref_customer;
        }
        ...
        if (empty($objectisused)) {
            ...
            if (!$error) {                
                if($this->deleteCustomerNanapet($temp_ref_customer)) {
                    $this->db->commit();
                    // Delete directory
    ...
    function LibStatut($statut, $mode = 0) {
        ...
        if ($mode == 2) {
            if ($statut == 0)
                return img_picto($langs->trans("ActivityCeased"), 'statut5') . ' ' . $langs->trans("ActivityCeased");
            if ($statut == 1)
                return img_picto($langs->trans("InActivity"), 'statut4') . ' ' . $langs->trans("InActivity");
        }
        if ($mode == 3) {
            if ($statut == 0)
                return img_picto($langs->trans("ActivityCeased"), 'statut5');
            if ($statut == 1)
                return img_picto($langs->trans("InActivity"), 'statut4');
        }
        if ($mode == 4) {
            if ($statut == 0)
                return img_picto($langs->trans("ActivityCeased"), 'statut5') . ' ' . $langs->trans("ActivityCeased");
        ...
        if ($mode == 5) {
            if ($statut == 0)
                return $langs->trans("ActivityCeased") . ' ' . img_picto($langs->trans("ActivityCeased"), 'statut5');
            if ($statut == 1)
                return $langs->trans("InActivity") . ' ' . img_picto($langs->trans("InActivity"), 'statut4');
        }
    }
    ...
    function id_prof_url($idprof, $soc) {
        ...
        if ($idprof == 1 && ($soc->country_code == 'GB' || $soc->country_code == 'UK'))
        ...
        if ($idprof == 1 && $soc->country_code == 'IN')
            $url = 'http://www.tinxsys.com/TinxsysInternetWeb/dealerControllerServlet?tinNumber=' . $soc->idprof1 . ';&searchBy=TIN&backPage=searchByTin_Inter.jsp';
    ...
    public function syncCustomerNanapet($user) {    
            ...
            if ($resql) {
                if ($this->dbNanapet->num_rows($resql) > 0) {
                    while ($obj = $this->dbNanapet->fetch_object($resql)) {
                        $dolibarrobject = new Societe($this->db, $this->db_nanapet);
                        $dolibarrobject->ref_customer = $this->dbNanapet->escape($obj->id);
                        $dolibarrobject->name = $this->dbNanapet->escape($obj->name);                                                
                        $dolibarrobject->name = str_replace("(Anh) ","", $dolibarrobject->name);
                        $dolibarrobject->name = str_replace("(Chị) ","", $dolibarrobject->name);
                        if($this->dbNanapet->escape($obj->sex) == "0") {
                            $temp_sex = "MLE";
                            $dolibarrobject->name = $this->dbNanapet->escape("(Chị) ".$dolibarrobject->name);
                        } else {
                            $temp_sex = "MR";
                            $dolibarrobject->name = $this->dbNanapet->escape("(Anh) ".$dolibarrobject->name);
                        }                        
                        $dolibarrobject->civility_id = $temp_sex;
                        $dolibarrobject->client = "1";
                        $dolibarrobject->phone = $this->dbNanapet->escape($obj->phone);
                        $dolibarrobject->email = $this->dbNanapet->escape($obj->email);
                        $dolibarrobject->address = str_replace("nanapet.com", "",
                        $this->dbNanapet->escape($obj->address)) ;
                        
                        // rowid Dolibarr customer
                        $temp_customer_id = $this->checkCustomerOnDolibarr($obj->id);
                        if ($temp_customer_id > 0) {
                            if ($dolibarrobject->syncUpdateCustomerOnDolibarr($temp_customer_id)) {
                                //do nothing
                            } else {
                                // return the rowid of customer, if it fail
                                // return $temp_customer_id;
                                return -1;
                            }
                        } else {
                            if ($dolibarrobject->create($user, true) > 0 ) {
                                // do nothing
                            } else {
                                // return if fail
                                // return $dolibarrobject->create($user, true);
                                return -1;
                            }                            
                        }
                    }
    ...
    // return the rowid of customer in Dolibarr OR return 0
    public function checkCustomerOnDolibarr($id) {
        $sql = "SELECT rowid";        
        $sql.= " FROM " . MAIN_DB_PREFIX . "societe";
        $sql.= " WHERE ref_customer = " . $this->db->escape($id);
        $resql = $this->db->query($sql);
        if ($resql) {
            if ($this->db->num_rows($resql) > 0) {
                $obj = $this->db->fetch_object($resql);
    ...
    public function syncUpdateCustomerOnDolibarr($id) {
        $this->db->begin();
        $sql = "UPDATE " . MAIN_DB_PREFIX . "societe";
        $sql.= " SET nom = '" . $this->db->escape($this->name) . "'";
        // $sql.= ", civility_id = '" . $this->db->escape($this->civility_id) . "'";
        $sql.= ", client = '" . $this->db->escape($this->client) . "'";
        $sql.= ", phone = '" . $this->db->escape($this->phone) . "'";
        $sql.= ", email = '" . $this->db->escape($this->email) . "'";
        $sql.= ", address = '" . $this->db->escape($this->address) . "'";
        $sql.= ", ref_customer = " . $this->db->escape($this->ref_customer);
        $sql.= " WHERE rowid = " . $id;
        dol_syslog(get_class($this) . "::update", LOG_DEBUG);
        $resql = $this->db->query($sql);
        if ($resql) {
    ...
    public function deleteAllCustomersDollibar() {
        $sql = "SELECT rowid ";
        $sql.= " FROM " . MAIN_DB_PREFIX . "societe";
        $resql = $this->db->query($sql);
        if ($resql) {
    ...
    public function insertCustomerNanapet($user) {
        /* DB NanaPet */
        if($this->dbNanapet != null){
            if($this->civility_id == "MME" || $this->civility_id == "MLE") {
                $sex = "0";
                $name = $this->dbNanapet->escape("(Chị) ".$this->name);
            } else {
                $sex = "1";
                $name = $this->dbNanapet->escape("(Anh) ".$this->name);
            }
            $phone = $this->dbNanapet->escape($this->phone);
            $email = $this->dbNanapet->escape($this->email);
            $address =  $this->dbNanapet->escape($this->address);
            ...
            if ($result) {
                ...
                if ($this->update($idNanaPet.",".$this->id_dolibarr, $user, 1, 0, 0,'updateNanaPet',1) > 0) {
    ...
    public function updateCustomerNanapet($id) {
        /* DB NanaPet */
        if($this->dbNanapet != null){
            // Get ref_customer on Dolibarr
            $temp_ref_nanapet = "";
            $sql = "SELECT ref_customer, nom";
            $sql.= " FROM " . MAIN_DB_PREFIX . "societe";
            if ($id) {
            ...
            if ($resql) {
                ...
                $temp_ref_nanapet = $obj->ref_customer;
            }
            
            $this->name = str_replace("(Anh) ","", $this->name);
            $this->name = str_replace("(Chị) ","", $this->name);
            if ($temp_ref_nanapet != "") {
                // Update NanaPet
                if($this->civility_id == "MME" || $this->civility_id == "MLE") {
                    $sex = "0";                    
                    $name = $this->db->escape("(Chị) ".$this->name);
                } else {
                    $sex = "1";
                    $name = $this->db->escape("(Anh) ".$this->name);
                }                                
    ...
    public function deleteCustomerNanapet($id) {
        if($this->dbNanapet != null){
            if ($id != 0) {
                $this->dbNanapet->begin();
    ...
    public function checkUpdateForCustomer() {
        if($this->dbNanapet != null && $this->db != null){
            $count_customer_nanapet = 0;
            $sql = "SELECT count(*) as count_customer"
                 . " FROM user";
            $result = $this->dbNanapet->query($sql);
            if($result) {
                if ($this->dbNanapet->num_rows($sql) > 0) {
                    $obj = $this->dbNanapet->fetch_object($sql);
                    $count_customer_nanapet = $obj->count_customer;
                }                                
            }
            
            $count_customer_dolibarr = 0;
            $sql = "SELECT count(*) as count_customer"
                .  " FROM llx_societe"
                .  " WHERE client = 1";
            ...
            if($result_dolibarr) {
                if ($this->db->num_rows($sql) > 0) {
                    ...
                    $count_customer_dolibarr = $obj->count_customer;
                }                                
            }            
            if($count_customer_nanapet > $count_customer_dolibarr) {
                return -1;
            } else if ($count_customer_nanapet < $count_customer_dolibarr){
                return 0;
            } else if ($count_customer_nanapet == $count_customer_dolibarr) {
                return 1;
            }
