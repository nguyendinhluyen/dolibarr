...
function showDP(base, dateFieldID, format, codelang)
{
    ...
    if (alreadybox)
        closeDPBox();	// This erase value of showDP.datefieldID
...
function resetDP(base, dateFieldID, format, codelang)
{
    ...
    var alreadybox = getObjectFromID("DPCancel");
    if (alreadybox)
        showDP(base, dateFieldID, format, codelang);
}
...
function loadMonth(base, month, year, ymd, codelang)
{
    ...
    if (req.responseText == '')
        alert('Failed to get URL '.theURL);
...
function dpChangeDay(dateFieldID, format)
{
    ...
    if (date)
    {
        ...
        if (thefieldday.onchange)
            thefieldday.onchange.call(thefieldday);
        thefieldmonth.value = date.getMonth() + 1;
        if (thefieldmonth.onchange)
            thefieldmonth.onchange.call(thefieldmonth);
        thefieldyear.value = date.getFullYear();
        if (thefieldyear.onchange)
            thefieldyear.onchange.call(thefieldyear);
    }
    else
    {
        thefieldday.value = '';
...
function dpClickDay(year, month, day, format)
{
    ...
    if (thefield.onchange)
        thefield.onchange.call(thefield);
...
function getDateFromFormat(val, format)
{
    ...
    while (i < format.length)
    {
        ...
        while ((format.charAt(j) == c) && (j < format.length))	// Recupere char
                ...
                // alert('substr='+substr);
                if (substr == "yyyy")
                    year = getIntegerInString(val, d, 4, 4);
                if (substr == "yy")
                    year = "" + (getIntegerInString(val, d, 2, 2) - 0 + 1900);
