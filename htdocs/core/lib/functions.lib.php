...
/**
 * 	\file			htdocs/core/lib/functions.lib.php
 * 	\brief			Function return format money
 */
...
function dol_get_fiche_head($links = array(), $active = '0', $title = '', $notab = 0, $picto = '', $pictoisfullpath = 0) {
    ...
    if (!empty($title) && $showtitle) {
        ...
        if ($picto)
            $out.=img_picto('', ($pictoisfullpath ? '' : 'object_') . $picto, '', $pictoisfullpath) . ' ';
...
function img_picto($titlealt, $picto, $options = '', $pictoisfullpath = false, $srconly = 0, $notitle = 0) {
    ...
    if ($srconly)
        ...
        if (preg_match('/:[^\s]/', $titlealt))
            $tmparray = explode(':', $titlealt);  // We explode if we have TextA:TextB. Not if we have TextA: TextB
        $title = $tmparray[0];
    ...
    if ($page > 0 || $num > $conf->liste_limit) {
        if ($totalnboflines) { // If we know total nb of lines
            ...
            if ($cpt >= 1) {
                ...
                if ($cpt >= 2)
                    $pagelist.='<li><span class="inactive">...</span></li>';
            }
            do {
                if ($cpt == $page) {
...
function print_fleche_navigation($page, $file, $options = '', $nextpage = 0, $betweenarrows = '', $afterarrows = '') {
    ...
    if ($page > 0) {
        if (empty($conf->dol_use_jmobile))
            print '<li class="pagination"><a class="paginationprevious" href="' . $file . '?page=' . ($page - 1) . $options . '"><</a></li>';
        else
            print '<li><a data-role="button" data-icon="arrow-l" data-iconpos="left" href="' . $file . '?page=' . ($page - 1) . $options . '">' . $langs->trans("Previous") . '</a></li>';
    }
    if ($betweenarrows) {
        print $betweenarrows;
    }
    if ($nextpage > 0) {
        if (empty($conf->dol_use_jmobile))
            print '<li class="pagination"><a class="paginationnext" href="' . $file . '?page=' . ($page + 1) . $options . '">></a></li>';
        else
            print '<li><a data-role="button" data-icon="arrow-r" data-iconpos="right" href="' . $file . '?page=' . ($page + 1) . $options . '">' . $langs->trans("Next") . '</a></li>';
    }
    if ($afterarrows) {
...
function price($amount, $form = 0, $outlangs = '', $trunc = 1, $rounding = -1, $forcerounding = -1, $currency_code = '') {
    ...
    $output = formatMoney(price2num($output, 1));
    return $output;
}
...
function dol_textishtml($msg, $option = 0) {
    if ($option == 1) {
        ...
        elseif (preg_match('/<body/i', $msg))
            return true;
        elseif (preg_match('/<(b|em|i)>/i', $msg))
            return true;
        elseif (preg_match('/<(br|div|font|li|span|strong|table)>/i', $msg))
            return true;
        elseif (preg_match('/<(br|div|font|li|span|strong|table)\s+[^<>\/]*>/i', $msg))
            return true;
        elseif (preg_match('/<(br|div|font|li|span|strong|table)\s+[^<>\/]*\/>/i', $msg))
            return true;
