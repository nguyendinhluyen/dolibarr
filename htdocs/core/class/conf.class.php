    ...
    function setValues($db) {
        ...
        if ($resql) {
            ...
            while ($i < $numr) {
                ...
                if ($key) {
                    if (!defined("$key"))
                        define("$key", $value); // In some cases, the constant might be already forced (Example: SYSLOG_HANDLERS during install)
        ...
        if (!isset($this->global->MAIN_MODULES_FOR_EXTERNAL))
            $this->global->MAIN_MODULES_FOR_EXTERNAL = 'user,askpricesupplier,facture,categorie,commande,fournisseur,contact,propal,projet,contrat,societe,ficheinter,expedition,agenda,adherent'; // '' means 'all'. Note that contact is added here as it should be a module later.
