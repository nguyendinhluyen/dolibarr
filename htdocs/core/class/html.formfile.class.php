...
class FormFile {
                ...
                foreach ($file_list as $file) {
                    ...
                    if (is_object($hookmanager)) {
                        ...
                        if (empty($res)) {
                            ...
                            $out = $hookmanager->resPrint;  // Replace line
                    }
                }
                ...
                if ($file['name'] != '.' && $file['name'] != '..' && !preg_match('/\.meta$/i', $file['name'])) {
                    ...
                    if (empty($relativepath)) {
                        ...
                        if ($object->element == 'member')
                            $relativepath = get_exdir($object->id, 2, 0, 0, $object, 'member') . $relativepath;    // TODO Call using a defined value for $relativepath
    ...
    public function listOfLinks($object, $permtodelete = 1, $action = null, $selected = null, $param = '') {
        ...
        print '<tr class="liste_titre">';
        print_liste_field_titre(
                $langs->trans("Links"), $_SERVER['PHP_SELF'], "name", "", $param, 'align="left"', $sortfield, $sortorder
        );
        print_liste_field_titre(
                "", "", "", "", "", 'align="right"'
        );
        print_liste_field_titre(
                $langs->trans("Date"), $_SERVER['PHP_SELF'], "date", "", $param, 'align="center"', $sortfield, $sortorder
        );
        print_liste_field_titre(
                '', $_SERVER['PHP_SELF'], "", "", $param, 'align="center"'
        );
        print_liste_field_titre('', '', '');
        print '</tr>';
