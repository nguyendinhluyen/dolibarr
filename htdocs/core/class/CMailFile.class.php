    ...
    function sendfile() {
        ...
        if (empty($conf->global->MAIN_DISABLE_ALL_MAILS)) {
            ...
            if ($conf->global->MAIN_MAIL_SENDMODE == 'mail') {
                ...
                // Forcage parametres
                if (empty($conf->global->MAIN_MAIL_SMTP_SERVER))
                    $conf->global->MAIN_MAIL_SMTP_SERVER = ini_get('SMTP');
                if (empty($conf->global->MAIN_MAIL_SMTP_PORT))
                    $conf->global->MAIN_MAIL_SMTP_PORT = ini_get('smtp_port');
            ...
            if ($email) {
                ...
                if ($format == 0 || $format == 3) {
                    ...
                    elseif (!$name)
                        ...
                        $newemail = ($format == 3 ? '"' : '') . ($encode ? $this->encodetorfc2822($name) : $name) . ($format == 3 ? '"' : '') . ' <' . $email . '>';
                }
