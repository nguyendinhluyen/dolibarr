...
// Clean parameters
    ...
    function printObjectLines($action, $seller, $buyer, $selected = 0, $dateSelector = 0) {
        ...
        // Define usemargins
        $usemargins = 0;
        if (!empty($conf->margin->enabled) && !empty($this->element) 
                && in_array($this->element, array('facture', 'propal', 'commande')))
            $usemargins = 1;

        print '<tr class="liste_titre nodrag nodrop">';
        // NanaPet - Show the line number
        $conf->global->MAIN_VIEW_LINE_NUMBER = 1;
        if (!empty($conf->global->MAIN_VIEW_LINE_NUMBER))
            print '<td align="center">'.$langs->trans('OrderNumber'). '</td>';

        // Description
        print '<td>' . $langs->trans('ProductName') . '</td>';

        // NanaPet - dateexpired
        print '<td align="right" id="print_detail_product" width="100">' . $langs->trans('DateExpiredShort') . '</td>';
        ...
        // Qty
        print '<td align="right" width="80">' . $langs->trans('Qty') . '</td>';

        if ($conf->global->PRODUCT_USE_UNITS) {
            print '<td align="left" width="80">' . $langs->trans('Unit') . '</td>';
        }

        // Reduction short
        print '<td align="right" width="80">' . $langs->trans('ReductionShort') . '</td>';

        if ($this->situation_cycle_ref) {
            print '<td align="right" width="80">' . $langs->trans('Progress') . '</td>';
        }
        ...
        // Total HT        
        print '<td align="right" width="100">' . $langs->trans('TotalMoney') . '</td>';
        print '<td id="print_detail_product"></td>';  // No width to allow autodim        
        print '<td id="print_detail_product"></td>';  // No width to allow autodim        
        print '<td id="print_detail_product"></td>';  // No width to allow autodim        
        print "</tr>\n";
    ...
    function printOriginLine($line, $var) {
        ...
        if ($conf->global->PRODUCT_USE_UNITS)
            $this->tpl['unit'] = $line->getLabelOfUnit('long');
