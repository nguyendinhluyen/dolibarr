...
class DolGraph {
    ...
    function SetData($data) {
        ...
        // NanaPet change name of month
        for ($i = 0; $i < count($this->data) ; $i++) {
            $this->data[$i][0] = "T" . (string)($i + 1);
        }
    }
    ...
    private function draw_artichow($file, $fileurl) {
        ...
        if (!isset($this->type[0]) || $this->type[0] == 'bars')
            $classname = 'BarPlot';    // Only one type (first one) is supported by artichow
        else if ($this->type[0] == 'lines')
            $classname = 'LinePlot';
        else
        ...
        // Other cases, graph of type 'bars', 'lines'
            ...
            while ($i < $nblot) {
                ...
                if (!isset($this->type[$i]) || $this->type[$i] == 'bars')
                    $this->stringtoshow.='bars: { show: true, align: "' . ($i == $firstlot ? 'center' : 'left') . '", barWidth: 0.5 }, ';
                if (isset($this->type[$i]) && $this->type[$i] == 'lines')
                    $this->stringtoshow.='lines: { show: true, fill: false }, ';
