...
//				)
    ...
    function select_element($table_element, $socid = 0, $morecss = '') {
        ...
        if (!in_array($table_element, array('don', 'expensereport_det', 'expensereport')))
        ...
        if (!in_array($table_element, array('expensereport_det')))
            $sql.= ' AND t.entity=' . getEntity('project');
        if ($linkedtothirdparty)
        ...
        if ($resql) {
            ...
            if ($num > 0) {
                ...
                if ($showempty)
                    $sellist.= '<option value="-1"></option>';
                if ($showallnone)
                    $sellist.= '<option value="all">--' . $langs->trans("Alls") . '--</option>';
                if ($showallnone)
                    $sellist.= '<option value="none">--' . $langs->trans("None") . '--</option>';
                while ($i < $num) {
                    $obj = $this->db->fetch_object($resql);
