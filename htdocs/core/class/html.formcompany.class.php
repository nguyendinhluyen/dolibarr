...
class FormCompany {
    ...
    function select_state($selected = '', $country_codeid = 0, $htmlname = 'state_id') {
        ...
        if ($result) {
            if (!empty($htmlname))
                $out.= '<select id="' . $htmlname . '" class="flat minwidth300" name="' . $htmlname . '">';
            if ($country_codeid)
            ...
            if ($num) {
                ...
                while ($i < $num) {
                    ...
                    if ($obj->code == '0') {  // Le code peut etre une chaine
                        ...
                        if ((!empty($selected) && $selected == $obj->rowid) || (empty($selected) && !empty($conf->global->MAIN_FORCE_DEFAULT_STATE_ID) && $conf->global->MAIN_FORCE_DEFAULT_STATE_ID == $obj->rowid)) {
