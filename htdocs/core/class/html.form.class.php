...
class Form {
    ...
    function textwithtooltip($text, $htmltext, $tooltipon = 1, $direction = 0, $img = '', $extracss = '', $notabs = 2, $incbefore = '', $noencodehtmltext = 0) {
        ...
        if ($tooltipon == 2 || $tooltipon == 3)
            $paramfortooltipimg = ' class="classfortooltip inline-block' . ($extracss ? ' ' . $extracss : '') . '" style="padding: 0px;' . ($extrastyle ? ' ' . $extrastyle : '') . '" title="' . ($noencodehtmltext ? $htmltext : dol_escape_htmltag($htmltext, 1)) . '"'; // Attribut to put on img tag to store tooltip
        else
            $paramfortooltipimg = ($extracss ? ' class="' . $extracss . '"' : '') . ($extrastyle ? ' style="' . $extrastyle . '"' : ''); // Attribut to put on td text tag
        if ($tooltipon == 1 || $tooltipon == 3)
            $paramfortooltiptd = ' class="classfortooltip inline-block' . ($extracss ? ' ' . $extracss : '') . '" style="padding: 0px;' . ($extrastyle ? ' ' . $extrastyle : '') . '" title="' . ($noencodehtmltext ? $htmltext : dol_escape_htmltag($htmltext, 1)) . '"'; // Attribut to put on td tag to store tooltip
        else
    ...
    function select_dolusers($selected = '', $htmlname = 'userid', $show_empty = 0, $exclude = '', $disabled = 0, $include = '', $enableonly = '', $force_entity = 0, $maxlength = 0, $showstatus = 0, $morefilter = '', $show_every = 0, $enableonlytext = '', $morecss = '') {
        ...
        if (is_array($include))
            ...
            $userid = $user->id;
            $include = array();
            if (empty($user->users) || !is_array($user->users))
                $user->get_full_tree();
            foreach ($user->users as $key => $val) {
                if (preg_match('/_' . $userid . '/', $val['fullpath']))
                    $include[] = $val['id'];
            }
            $includeUsers = implode("','", $include);
        ...
        if ($resql) {
            ...
            if ($num) {
                ...
                if ($show_empty)
                    $out.= '<option value="-1"' . ((empty($selected) || $selected == -1) ? ' selected' : '') . '>&nbsp;</option>' . "\n";
                if ($show_every)
                    $out.= '<option value="-2"' . (($selected == -2) ? ' selected' : '') . '>-- ' . $langs->trans("Everybody") . ' --</option>' . "\n";
    ...
    function select_produits_list($selected = '', $htmlname = 'productid', $filtertype = '', $limit = 20, $price_level = 0, $filterkey = '', $status = 1, $finished = 2, $outputmode = 0, $socid = 0) {
        ...
        if (!empty($conf->global->PRODUIT_CUSTOMER_PRICES_BY_QTY)) {
            ...
            if ($price_level >= 1 && !empty($conf->global->PRODUIT_MULTIPRICES))
        ...
        $sql.= ' WHERE p.stock > 0 AND p.entity IN (' . getEntity('product', 1) . ')';
        if ($finished == 0) {
        ...
        if ($filterkey != '') {
            ...
            foreach ($scrit as $crit) {
                ...
                if (!empty($conf->global->MAIN_MULTILANGS))
                    $sql.=" OR pl.label LIKE '" . $prefix . $crit . "%'";
                $sql.=")";
                $i++;
            }
            if (count($scrit) > 1)
                $sql.=")";
            if (!empty($conf->barcode->enabled))
                $sql.= " OR p.barcode LIKE '" . $prefix . $filterkey . "%'";
            $sql.=')';
        }
        $sql.= $db->order("p.ref");
        // $sql.= $db->plimit($limit);
        ...
        if ($result) {
            ...
            $out.='<select class="flat" name="' . $htmlname . '" id="' . $htmlname . '" autofocus style="margin-top: 10px">';
            ...
            while ($num && $i < $num) {
                ...
                if (!empty($objp->price_by_qty) && $objp->price_by_qty == 1 && !empty($conf->global->PRODUIT_CUSTOMER_PRICES_BY_QTY)) { // Price by quantity will return many prices for the same product
    ...
    private function constructProductListOption(&$objp, &$opt, &$optJson, $price_level, $selected) {
        ...
        if (!$found) {
            if ($objp->price_base_type == 'HT') {
                ...
                $opt.= price($objp->price_ttc, 1, $langs, 0, 0, -1, $conf->currency);
        ...
        if ($htmlname != "none") {
            ...
            if (!empty($conf->global->FACTURE_DEPOSITS_ARE_JUST_PAYMENTS)) {
                if (!$filter || $filter == "fk_facture_source IS NULL")
                    print $langs->trans("CompanyHasAbsoluteDiscount", price($amount, 0, $langs, 0, 0, -1, $conf->currency)) . ': ';    // If we want deposit to be substracted to payments only and not to total of final invoice
                else
    ...
    function load_tva($htmlname = 'tauxtva', $selectedrate = '', $societe_vendeuse = '', $societe_acheteuse = '', $idprod = 0, $info_bits = 0, $type = '', $options_only = false) {
        ...
        if ($num > 0) {
            ...
            if (!$options_only)
                $return.= '<select class="flat" id="' . $htmlname . '" name="' . $htmlname . '"' . ($disabled ? ' disabled' : '') . $title . '>';
    ...
    static function showphoto($modulepart, $object, $width = 100, $height = 0, $caneditfield = 0, $cssclass = 'photowithmargin') {
        ...
        if ($modulepart == 'societe') {
            ...
            if ($object->photo)
                $file = get_exdir($id, 2, 0, 0, $object, 'invoice_supplier') . 'photos/' . $object->photo;
            if (!empty($conf->global->MAIN_OLD_IMAGE_LINKS))
                $altfile = $object->id . ".jpg"; // For backward compatibility
            $email = $object->email;
        } else {
            ...
            if ($object->photo)
                $file = get_exdir($id, 2, 0, 0, $adherent, 'member') . 'photos/' . $object->photo;
            if (!empty($conf->global->MAIN_OLD_IMAGE_LINKS))
                $altfile = $object->id . ".jpg"; // For backward compatibility
            $email = $object->email;
        }
    ...
    function select_dolgroups($selected = '', $htmlname = 'groupid', $show_empty = 0, $exclude = '', $disabled = 0, $include = '', $enableonly = '', $force_entity = 0) {
        ...
        if ($resql) {
            ...
            if ($num) {
                if ($show_empty)
                    $out.= '<option value="-1"' . ($selected == -1 ? ' selected' : '') . '>&nbsp;</option>' . "\n";
