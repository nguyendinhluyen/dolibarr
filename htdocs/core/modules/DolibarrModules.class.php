...
// Remove list of module's cron job entries (entry in llx_cronjobs)
    ...
    function getVersion() {
        ...
        if ($newversion == 'experimental')
            $ret = $langs->trans("VersionExperimental");
        elseif ($newversion == 'development')
            $ret = $langs->trans("VersionDevelopment");
        elseif ($newversion == 'dolibarr')
            $ret = DOL_VERSION;
        elseif ($newversion)
            $ret = $newversion;
        else
            $ret = $langs->trans("VersionUnknown");

        if (preg_match('/_deprecated/', $this->version))
            $ret.=' (' . $langs->trans("Deprecated") . ')';
        return $ret;
    }
    ...
    function isCoreOrExternalModule() {
        ...
        if (!empty($this->version) && !in_array($this->version, array('experimental', 'development')))
            return 'external';
        if (!empty($this->editor_name) || !empty($this->editor_web))
            return 'external';
        return 'unknown';
    }
