...
<tr class="liste_titre nodrag nodrop">
    ...
    <td align="right">
        <!--<span id="title_up_ht"><?php //echo $langs->trans('PriceUHT'); ?></span>-->        
        <span id="title_date_expired"><?php echo $langs->trans('DateExpiredShort'); ?></span>
    </td>
...
<tr <?php echo $bcnd[$var]; ?>>
    ...
    <td class="nobottom"<?php echo (!empty($conf->global->MAIN_VIEW_LINE_NUMBER) ? ' colspan="2"' : ''); ?>>
        ...
        if (!empty($conf->product->enabled) || !empty($conf->service->enabled)) {
            ...
            echo '<input type="radio" name="prod_entry_mode" id="prod_entry_mode_predef" value="predef"' 
            . (GETPOST('prod_entry_mode') == 'predef' ? ' checked' : '') . '> ';
            if (empty($senderissupplier)) {
                if (!empty($conf->product->enabled) && empty($conf->service->enabled))
    ...
    <td class="nobottom" align="right">
        <!--<input type="text" size="5" name="price_ht" id="price_ht" class="flat" value="
            <?php //echo (isset($_POST["price_ht"]) ? $_POST["price_ht"] : ''); ?>">-->       
        <!--NanaPet get info of product-->
        <select id="select_date_expired" name="select_date_expired">    
             <option value="-1">0000-00-00 Tồn kho: 0</option>
        </select>
    </td>    
    <?php if (!empty($inputalsopricewithtax)) { ?>
        <td class="nobottom" align="right">
    ...
    <?php } ?>
    <td class="nobottom" align="right">
        <input style="text-align: right" type="text" size="2" name="qty" id="qty" class="flat" value="<?php echo (isset($_POST["qty"]) ? $_POST["qty"] : 1); ?>">
    </td>
    <?php
    if ($conf->global->PRODUCT_USE_UNITS) {
    ...
    ?>
    <td class="nobottom nowrap" align="right">
        <input type="text" size="1" style="text-align: right" name="remise_percent" id="remise_percent" class="flat" value="<?php echo (isset($_POST["remise_percent"]) ? $_POST["remise_percent"] : $buyer->remise_percent); ?>"><span class="hideonsmartphone">%</span></td>
    <?php
    if ($this->situation_cycle_ref) {
        $coldisplay++;
                    ...
                    if (data && data.length > 0)
                    {
                        ...
                        $(data).each(function () {
                            if (this.id != 'pmpprice')
                            {
                                ...
                                if (i == 1 && this.price > 0 && 'bestsupplierprice' == defaultbuyprice) {
                                    defaultkey = this.id;
...
<?php } ?>
    ...
    function setforfree() {
        ...
        // NanaPet get info of product
        jQuery('#select_date_expired').hide();
        jQuery("#title_date_expired").hide();
    }
    function setforpredef() {
        ...
        // NanaPet get info of product
        jQuery("#title_date_expired").show();
        jQuery('#select_date_expired').show();
        var product = "";
        var idprod = jQuery('#idprod').val();        
        var entrepot_id = $('#entrepot_id').val();        
        var getProductDetail = function() {
            $.ajax({
                type: "POST",
                url: '<?php echo DOL_URL_ROOT; ?>/commande/ajax/productdetail.php',
                data: {idproduct:idprod, entrepot_id: entrepot_id},
                async : false,
        ...
        var objs = JSON.parse(product);
        jQuery("#select_date_expired").empty();
        var select_date = document.getElementById("select_date_expired");        
        if( objs.length > 0 ) {
            for (i = 0 ; i < objs.length; i++) {
                var option = document.createElement("option");
                option.value = objs[i];
                option.text = objs[i];
                select_date.add(option);
            }
        } else {
            var option = document.createElement("option");
            option.value = "0000-00-00";
            option.text = "0000-00-00 Tồn kho: 0";
            select_date.add(option);
        }        
    }
</script>
