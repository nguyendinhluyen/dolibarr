 ...
 * Copyright (C) 2010-2015 Juanjo Menent        <jmenent@2biyte.es>
...
class Commande extends CommonOrder {
    // NanaPet
    public $fk_entrepot;
    public $entrepot_label;
    public $entrepot_lieu; 
    public $entrepot_id;
    public $chk_received_order;
    public $recieved_name;
    public $recieved_phone;
    public $recieved_address;
    public $discount;
    public $fee_ship;
    public $note_order;
     ...
     */
    function update_warehouse($user) {
        $product = new Product($this->db);
        $id_entrepot = 0;
        $movement = 1;
        for ($i = 0; $i < count($this->lines) ; $i++) {
            $product->id = $this->lines[$i]->proid;
            $sql = "SELECT rowid, fk_entrepot, reel"
                . " FROM " . MAIN_DB_PREFIX . "product_stock"
                . " WHERE fk_product = ".$product->id
                . " AND fk_entrepot = ".$this->entrepot_id
                . " AND dateexpired = '".$this->lines[$i]->date_expired."'";
            $resql = $this->db->query($sql);
            if ($resql) {
                while ($obj = $this->db->fetch_object($resql)) {
                    $id_entrepot = $obj->fk_entrepot;
                    $reel = $obj->reel;
                    if ($id_entrepot > 0 && $reel > 0) {
                        // Substract product on ware house
                        $product->correct_stock($user, $id_entrepot, $this->lines[$i]->qty, 
                                $movement, '', 0,
                                '', $this->lines[$i]->date_expired);
                        break;
                    } else {
                        return -100;
                    }
    ...
    // Check products of stock for validate
    function check_warehouse() {
        for ($i = 0; $i < count($this->lines) ; $i++) {            
            $sql = "SELECT rowid, fk_entrepot, reel, dateexpired"
                . " FROM " . MAIN_DB_PREFIX . "product_stock" 
                . " WHERE fk_product = ".$this->lines[$i]->proid
                . " AND fk_entrepot = ".$this->entrepot_id
                . " AND dateexpired = '".$this->lines[$i]->date_expired."'";
            $resql = $this->db->query($sql);
            if ($resql) {
                while ($obj = $this->db->fetch_object($resql)) {                    
                    $reel = $obj->reel;
                    $date_expired = $obj->dateexpired;
                    $fk_products = $this->lines[$i]->proid;
                    $number_product_stock = 0;
                    for ($j = 0; $j < count($this->lines) ; $j++) {
                        if ($this->lines[$j]->proid === $fk_products 
                                && $this->lines[$j]->date_expired === $date_expired) {
                            $number_product_stock += $this->lines[$j]->qty;
                        }
                    }
                    if ($reel < $number_product_stock) {
                      return -100;  
                    }
                }
                $this->db->free($resql);
    ...
    function create($user, $notrigger = 0) {
        ...
        $sql.= ", received_name";
        $sql.= ", received_phone";
        $sql.= ", received_address";
        $sql.= ", note_order";
        $sql.= ", ship";
        $sql.= ", discount";
        $sql.= ", chk_received_order";
        $sql.= ")";
        ...
        $sql.= ", '" . $this->recieved_name ."'";
        $sql.= ", '" . $this->recieved_phone ."'";
        $sql.= ", '" . $this->recieved_address ."'";
        $sql.= ", '" . $this->note_order ."'";
        $sql.= ", " . $this->fee_ship;
        $sql.= ", " . $this->discount;
        $sql.= ", " . $this->chk_received_order;
        $sql.= ")";
            ...
            if ($this->id) {
                ...
                for ($i = 0; $i < $num; $i++) {
                    ...
                    $result = $this->addline(
                            ...
                            $this->lines[$i]->fk_remise_except, 'HT', 0, 
     ...
     */
    function addline($desc, $pu_ht, $qty, $txtva, 
            $txlocaltax1 = 0, $txlocaltax2 = 0, $fk_product = 0, 
            $remise_percent = 0, $info_bits = 0, $fk_remise_except = 0, 
            $price_base_type = 'HT', $pu_ttc = 0, $date_start = '', 
            $date_end = '', $type = 0, $rang = -1, $special_code = 0, 
            $fk_parent_line = 0, $fk_fournprice = null, $pa_ht = 0, 
            $label = '', $array_options = 0, $fk_unit = null, $date_expired = "") {
        ...
        if ($this->statut == self::STATUS_DRAFT) {
            ...
            $tabprice = calcul_price_total($qty, $pu,
                    $remise_percent, $txtva, $txlocaltax1,
                    $txlocaltax2, 0, $price_base_type,
                    $info_bits, $type, '', $localtaxes_type);
            $total_ht = round(price2num($tabprice[0])/1000) * 1000;
            $total_tva = $tabprice[1];
            $total_ttc = round(price2num($tabprice[2])/1000) * 1000;
            ...
            $this->line->date_expired= $date_expired;
    ...
    function fetch($id, $ref = '', $ref_ext = '', $ref_int = '') {
        ...
        $sql.= ', c.received_name as received_name';
        $sql.= ', c.received_phone';
        $sql.= ', c.received_address';
        $sql.= ', c.note_order';
        $sql.= ', c.ship';
        $sql.= ', c.discount';
        $sql.= ', c.chk_received_order';
        $sql.= ", i.libelle as libelle_incoterms";
        $sql.= ', p.code as mode_reglement_code, p.libelle as mode_reglement_libelle';
        $sql.= ', cr.code as cond_reglement_code, cr.libelle as cond_reglement_libelle, cr.libelle_facture as cond_reglement_libelle_doc';
        $sql.= ', ca.code as availability_code';
        $sql.= ', dr.code as demand_reason_code';
        $sql.= ', en.label as entrepot_label';
        $sql.= ', en.lieu as entrepot_lieu';
        $sql.= ', en.rowid as entrepot_id';
        ...
        $sql.= ' LEFT JOIN ' . MAIN_DB_PREFIX . 'entrepot as en ON c.fk_entrepot = en.rowid';
        ...
        if ($result) {
            ...
            if ($obj) {
                ...
                $this->entrepot_lieu = $obj->entrepot_lieu;
                $this->entrepot_id = $obj->entrepot_id;
                $this->recieved_name = $obj->received_name; 
                $this->recieved_address = $obj->received_address; 
                $this->recieved_phone = $obj->received_phone; 
                $this->note_order = $obj->note_order;
                $this->ship = $obj->ship;
                $this->discount = $obj->discount;
                $this->chk_received_order = $obj->chk_received_order;
    ...
    function classifyBilled() {
        ...
        $sql = 'UPDATE ' . MAIN_DB_PREFIX . 'commande SET facture = 1, fk_statut = 4';
        $sql.= ' WHERE rowid = ' . $this->id . ' AND fk_statut > ' . self::STATUS_DRAFT;
    ...
    function updateline($rowid, $desc, $pu, $qty, $remise_percent, $txtva, $txlocaltax1 = 0.0, $txlocaltax2 = 0.0, $price_base_type = 'HT', $info_bits = 0, $date_start = '', $date_end = '', $type = 0, $fk_parent_line = 0, $skip_update_total = 0, $fk_fournprice = null, $pa_ht = 0, $label = '', $special_code = 0, $array_options = 0, $fk_unit = null) {
        ...
        if (!empty($this->brouillon)) {
            ...
            $tabprice = calcul_price_total($qty, $pu, $remise_percent, 
                    $txtva, $txlocaltax1, $txlocaltax2, 0, 
                    $price_base_type, $info_bits, $type, 
    ...
    function getLinesArray() {
        ...
        if ($resql) {
            ...
            while ($i < $num) {
                ...
                $this->lines[$i]->proid = $obj->prodid;
                $this->lines[$i]->date_expired = $obj->dateexpired;
                $i++;
            }
            $this->db->free($resql);
            return 1;
     ...
     */
    // NanaPet - Remove this method because of not supporting to print UTF-8 document
     ...
     * @param 
     */
    // NanaPet - Use for printing bill in commande 
    // Using print.css.php file for custome bill
    public function printBill($order_name="",$order_phone="", $order_address="", $chk_received_order=0,
                              $receive_name="", $receive_phone="", $receive_address="",
                              $note="", $billed) {
        global $langs;
        $langs->load('orders');
        // Header info
        print '<table class="header_bill" width="100%">';
        print '<tr>';
        print '<td style="width: 50%" rowspan="3">';
        print '<img src='. DOL_URL_ROOT . '/commande/style/logo.png alt="nanapet logo" width="200px">';
        print '</td>';
        print '<td class="header_title">';
        print 'Chuyên cung cấp thức ăn, vật dụng cho chó mèo';
        print '</td></tr>';
        print '<tr>';
        print '<td class="info_title">';
        print 'www.nanapet.com (08) 2212.8888 - 0933.103.203';
        print '</td></tr>';
        print '<tr>';
        print '<td class="info_title">';
        print 'Facebook/NanaPetOnline support@nanapet.com';
        print '</td></tr>';
        print '</table>';
        
        // Customer info
        print '<table class="customer_bill" width="100%">';
        // Order info
        print '<tr>';
        print '<td class="title_name">';
        print '<b>Người Đặt Hàng : </b>';
        print '</td>';
        print '<td class="name">';
        print $order_name;
        print '</td>';
        print '<td class="title_phone">';
        print '<b>Số Điện Thoại : </b>';
        print '</td>';
        ...
        print '</tr>';
        print '<tr>';
        print '<td class="address" colspan ="4">';
        print '<b>Địa chỉ : </b>';
        print '<span>'.$order_address.'</span>';
        print '</td>';
        print '</tr>';
        // End order info
        // Receive info
        if ($chk_received_order == 0) {
            print '<tr>';
            print '<td class="title_name">';
            print '<b>Người Nhận Hàng : </b>';
            print '</td>';
            print '<td class="name">';
            print $receive_name;
            print '</td>';
            ...
            print '</tr>';  
            print '<tr>';
            print '<td class="email" colspan ="4">';
            print '<b>Địa chỉ : </b>';
            print '<span>'.$receive_address.'</span>';
            print '</td>';
            print '</tr>';
            // End received info
        }
        ...
        // Info order bill 
        print '<table class="order_received_bill" width="100%">';    
        print '<tr>';
        print '<td class="check_box_order_received">';
        if ($chk_received_order == 1) {
            print '<input type="checkbox" name="check_order_received" checked disabled '
            . 'style="font-size: 110%">&nbsp;Người đặt hàng là người nhận hàng</input>' ;
        } else {
            print '<input type="checkbox" name="check_order_received" disabled '
            . 'style="font-size: 110%">&nbsp;Người đặt hàng là người nhận hàng</input>' ;
        }
        print '</td>';
        print '<td class="order">';
        print 'Đơn hàng';
        print '</td>';
        if ($billed == Commande::STATUS_BILLED) {
            print '<td class="order_bill_not_done">';
            print '<input type="checkbox" name="check_bill_done" style="font-size: 110%" checked disabled>'
            . '&nbsp;đã thanh toán</input>' ;
            print '</td>';
        } else {
            print '<td class="order_bill_not_done">';
            print '<input type="checkbox" name="check_bill_done" style="font-size: 110%" disabled>'
            . '&nbsp;đã thanh toán</input>' ;
            print '</td>';
        }
        
        if ($billed < Commande::STATUS_BILLED) {
            print '<td class="order_bill_done">';
            print '<input type="checkbox" name="check_bill_not_done" style="font-size: 110%" checked disabled>'
            . '&nbsp;chưa thanh toán</input>' ;
            print '</td>';
        } else {
            print '<td class="order_bill_done">';
            print '<input type="checkbox" name="check_bill_not_done" style="font-size: 110%" disabled>'
            . '&nbsp;chưa thanh toán</input>' ;
            print '</td>';
        }
        ...
        // Begin note bill
        print '<table class="note_bill" width="100%">';
        print '<tr><td>';
        print '<u>Ghi chú </u>: ';
        print $note;
        print '</td></tr>';
    ...
    //Nanapet - total money
    public function printBillMoney ($total_order=0, $fee_ship=0, $discount=0, 
                                    $total_money=0, $number_products="") {
        // Begin money
        // Begin total order
        print '<table class="ship_bill" width="50%" style="padding-top:20px">';  
        print '<tr>';
        print '<td class="title_ship">';
        print 'Tổng tiền đơn hàng';
        print '</td>';
        print '<td class="plus_symbol_ship">';
        print '+';
        print '</td>';
        print '<td class="fee_ship">';
        print $total_order;
        print '</td>';
        print '</tr>';
        // End total order
        ...
        print '</tr>';
        // End discount
        // Begin total moneyprint
        print '<tr>';
        print '<td class="title_money">';
        print 'Tổng cộng';
        print '</td>';
        print '<td class="number_product">';
        print $number_products. " món";
        print '</td>';
        print '<td class="money">';
        print $total_money;
        print '</td>';
        print '</tr>';
        print '</table>';
