...
// NanaPet - Custommize for printer
require_once DOL_DOCUMENT_ROOT . '/commande/style/print.css.php';
...
$chk_received_order = GETPOST('chk_received_order', 'boolean');
$received_name = GETPOST('received_name', 'alpha'); 
$received_phone = GETPOST('received_phone', 'alpha'); 
$received_address = GETPOST('received_address', 'alpha'); 
$ship = GETPOST('ship', 'alpha');
if (empty($ship)) {
    $ship = 0;
}
$discount = GETPOST('discount', 'alpha');
if (empty($discount)) {
    $discount = 0;
}
$note_order = GETPOST('note_order', 'alpha'); 
...
$object_client = new Client($db,$db_nanapet);

// fetch optionals attributes and labels
$extralabels = $extrafields->fetch_name_optionals_label($object->table_element);

// Load object - fetch object
...
if (empty($reshook)) {
    ...
    // Add order
        ...
        if ($socid < 1) {
            setEventMessage($langs->trans("ErrorFieldRequired", $langs->transnoentitiesnoconv("Customer_Order")), 'errors');
            $action = 'create';
            $error++;
        }
        
        if ($chk_received_order == false) {
            if (empty($received_name)) {
                setEventMessage($langs->trans('ErrorFieldRequired', $langs->transnoentities('ReceivedName')), 'errors');
                $action = 'create';
                $error++;
            }
            if (empty($received_phone)) {
                setEventMessage($langs->trans('ErrorFieldRequired', $langs->transnoentities('ReceivedPhone')), 'errors');
                $action = 'create';
                $error++;
            }
            if (empty($received_address)) {
                setEventMessage($langs->trans('ErrorFieldRequired', $langs->transnoentities('ReceivedAddress')), 'errors');
                $action = 'create';
                $error++;
            }
        }

        // Check ship always is the number and positive
        if (!empty($ship) && $ship !== 0) {
            if (!is_numeric($ship)) {
                setEventMessage($langs->trans('ErrorFieldRequired', $langs->transnoentities('ShipWrong')), 'errors');
                $action = 'create';
                $error++;
            } else if (is_numeric($ship) && $ship < 0) {
                setEventMessage($langs->trans('ErrorFieldRequired', $langs->transnoentities('ShipWrong')), 'errors');
                $action = 'create';
                $error++;
            }
        ...
        // Check discount always is the number and positive
        if (!empty($discount) && $discount !== 0) {
            if (!is_numeric($discount)) {
                setEventMessage($langs->trans('ErrorFieldRequired', $langs->transnoentities('DiscountWrong')), 'errors');
                $action = 'create';
                $error++;
        ...
        if (empty($datecommande)) {
            setEventMessage($langs->trans('ErrorFieldRequired', $langs->transnoentities('OrderDate')), 'errors');
            $action = 'create';
            $error++;
        }
        ...
        if (!$error) {
            // NanaPet
            $object->fk_entrepot = $id_entrepot;
            if ($chk_received_order == true) {
                $object->chk_received_order = "1";
            } else {
                $object->chk_received_order = "0";
            }
            
            $object->recieved_name = $received_name;
            $object->recieved_phone = $received_phone;
            $object->recieved_address = $received_address;
            $object->fee_ship = $ship;
            $object->discount = $discount;
            $object->note_order = $note_order;
    ...
    } else if ($action == 'classifybilled' && GETPOST('optioncss', 'alpha') == ""
            && $user->rights->commande->creer) {
        $soc = new Societe($db);
        if ($object->id > 0) {
            $soc->fetch($object->socid);
        }
        if($object_client->addScoreForCustomer($soc->email, $object->total_ttc) > 0) {
            $ret = $object->classifyBilled();
            if ($ret < 0) {
                setEventMessage($object->error, 'errors');
            } else {
                $object->getLinesArray();
                setEventMessage($langs->trans("UpdateScore"));
            }
        } else {
            setEventMessage($langs->trans("ErrorAddScore"), 'errors');
        }                
    }
    ...
    // Add a new line
        ...
        $error = 0;
        // NanaPet
        $date_expired = explode(" ",GETPOST("select_date_expired"))[0];
        $stock_number = explode(" ",GETPOST("select_date_expired"))[3];
        ...
        if ($stock_number <= 0){
            setEventMessage($langs->trans('ErrorQuantity'),'errors');
            $error++;
        }
        
        if ($stock_number > 0 &&  $stock_number < $qty ){
            setEventMessage($langs->trans('ErrorAddQuantity'),'errors');
            $error++;
        }
        ...
        if (!$error && ($qty >= 0) && (!empty($product_desc) || !empty($idprod))) {
            ...
            if (!empty($idprod)) {
                ...
                else if (!empty($conf->global->PRODUIT_CUSTOMER_PRICES)) {
            ...
            if (!empty($price_min) && (price2num($pu_ht) * (1 - price2num($remise_percent) / 100) < price2num($price_min))) {
                ...
                // Insert line
                $result = $object->addline($desc, $pu_ht, $qty, 
                        $tva_tx, $localtax1_tx, $localtax2_tx, 
                        $idprod, $remise_percent, $info_bits, 0, 
                        $price_base_type, $pu_ttc, $date_start, 
                        $date_end, $type, - 1, 0, 
                        GETPOST('fk_parent_line'), $fournprice, 
                        $buyingprice, $label, $array_options, $fk_unit, 
                        $date_expired);
                if ($result > 0) {
                    $ret = $object->fetch($object->id); // Reload to get new records
     ...
     */ else if ($action == 'updateline' && $user->rights->commande->creer && GETPOST('save')) {
        ...
        exit();
    } else if ($action == 'confirm_validate' && GETPOST('optioncss', 'alpha') == ""
        && $confirm == 'yes'
        &&((empty($conf->global->MAIN_USE_ADVANCED_PERMS)
        && !empty($user->rights->commande->creer)) || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) 
        && !empty($user->rights->commande->order_advance->validate)))) {
        ...
        if (!$error) {
            // NanaPet - Update warehouse
            $object->getLinesArray();
            if ($object->update_warehouse($user) > 0) {
                ...
                if ($result >= 0) {
                    ...
                    setEventMessages($langs->trans("UpdateWarehouse"), null, 'mesgs');                
                }                                 
                else {
                    setEventMessages($object->error, $object->errors, 'errors');
                }                       
            } else {
                setEventMessages($langs->trans("ErrorUpdateWarehouse"), null, 'errors');
                $error++;
            }                                         
        }
    }
    ...
    // Go back to draft status
        ...
        if ($result < 0) {
            ...
            ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->commande->creer)) || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->commande->order_advance->validate)))
    ) {
        $idwarehouse = GETPOST('idwarehouse');
...
if ($action == 'create' && $user->rights->commande->creer) {
    ...
    print '<tr><td width="25%" class="fieldrequired">'
            . $langs->trans('Ref') . "<span style='color:red'> *<span>"
            . '</td><td colspan="2">' . $langs->trans("Draft") . '</td></tr>';
    ...
    // NanaPet - Stock product
    print '<tr>';
    print '<td class="fieldrequired">' . $langs->trans('Warehouse') 
    ...
    // Client
    print '<tr>';
    print '<td class="fieldrequired">' . $langs->trans('Customer_Order') 
            . "<span style='color:red'> *<span>".'</td>';
    if ($socid > 0) {
        print '<td colspan="2">';
    ...
    if ($socid > 0) {
        ...
        if ($absolute_discount)
            print $langs->trans("CompanyHasAbsoluteDiscount", price($absolute_discount)
    ...
    // Received customer infos 
    print '<tr>';
    print '<td/>';
    if ($chk_received_order == true) {
        print '<td><input type="checkbox" name="chk_received_order" id="chk_received_order" '
        . 'style="font-size: 110%" checked onchange="eventClickCheckbox();"/>';
        print '&nbsp'.$langs->trans("DefaultReceived");
        print '</td></tr>';
        // Received name
        print '<tr><td class="fieldrequired">' . $langs->trans('ReceivedName')
                . "<span style='color:red'> *<span>"
                .'</td><td colspan="2">';
        print '<input type="text" name="received_name" id="received_name" style="width: 309px" disabled></td>';
        print '</tr>';
        // Received phone
        print '<tr><td class="fieldrequired">' . $langs->trans('ReceivedPhone')
                ."<span style='color:red'> *<span>" . '</td><td colspan="2">';
        print '<input type="text" name="received_phone" id="received_phone" disabled></td>';
        print '</tr>';
        // Received address
        print '<tr><td class="fieldrequired">' . $langs->trans('ReceivedAddress') 
                . "<span style='color:red'> *<span>" . '</td><td colspan="2">';
        // print '<input type="text" name="received_address" id="received_address" style="width: 200px" disabled>';
        $doleditor = new DolEditor('received_address', $note_public, '', 80, 'received_address', 'In', 0, false, true, ROWS_3, '90%');
        print $doleditor->Create(1);
        print '</td>';
        print '</tr>';     
        ?><script>document.getElementById("received_address").disabled=true;</script><?php
    } else {
        print '<td><input type="checkbox" name="chk_received_order" id="chk_received_order" '
        . 'style="font-size: 110%" onchange="eventClickCheckbox();"/>';
        print '&nbsp'.$langs->trans("DefaultReceived");
        print '</td></tr>';
        // Received name
        print '<tr><td class="fieldrequired">' . $langs->trans('ReceivedName')
                . "<span style='color:red'> *<span>"
                .'</td><td colspan="2">';
        print '<input type="text" name="received_name" id="received_name" style="width: 309px"'
                . 'value = "'.$received_name.'"></td>';
        print '</tr>';
        // Received phone
        print '<tr><td class="fieldrequired">' . $langs->trans('ReceivedPhone')
                ."<span style='color:red'> *<span>" . '</td><td colspan="2">';
        print '<input type="text" name="received_phone" id="received_phone" '
                . 'value="'.$received_phone.'"></td>';
        print '</tr>';
        // Received address
        print '<tr><td class="fieldrequired">' . $langs->trans('ReceivedAddress') 
                . "<span style='color:red'> *<span>" . '</td><td colspan="2">';
        //print '<input type="text" name="received_address" id="received_address" style="width: 200px">';
        $doleditor = new DolEditor('received_address', $note_public, '', 80, 'received_address', 'In', 0, false, true, ROWS_3, '90%');
        $doleditor->content = $received_address;
        print $doleditor->Create(1);
        print '</td>';
        print '</tr>';
    }
    ...
    // Ship
    print '<tr><td class="fieldrequired">' . $langs->trans('Ship')
            ."<span style='color:red'> *<span>" . '</td><td colspan="2">';
    print '<input type="text" name="ship" style="width: 200px" value= "'.$ship.'"></td>';
    print '</tr>';
     // Discount
    print '<tr><td>' . $langs->trans('DiscountOrder') . '</td><td colspan="2">';
    print '<input type="text" name="discount" style="width: 200px" value="'.$discount.'"></td>';
    print '</tr>';
    // Note order
    print '<tr>';
    print '<td class="border" valign="top">' . $langs->trans('NoteOrder') . '</td>';
    print '<td valign="top" colspan="2">';
    $doleditor = new DolEditor('note_order', $note_public, '', 80, 'note_order', 'In', 0, false, true, ROWS_3, '90%');
    $doleditor->content= $note_order;
    print $doleditor->Create(1);
    // print '<textarea name="note_public" wrap="soft" cols="70" rows="'.ROWS_3.'">'.$note_public.'</textarea>';
    print '</td></tr>';
    // End order customer infos
    
    // Date  
    print '<tr><td class="fieldrequired">' . $langs->trans('OrderDate') 
            ."<span style='color:red'> *<span>" .  '</td><td colspan="2">';
    ...
    if ($object->id > 0) {
        ...
        // NanaPet - Check status for printing tab header 
        if ($optioncss !== "print") {
        ...
        if ($action == 'delete') {
            $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] 
                    . '?id=' . $object->id, $langs->trans('DeleteOrder'), 
                    $langs->trans('ConfirmDeleteOrder'), 'confirm_delete', '', 0, 1);
        }
        ...
        if ($action == 'validate') {
            // NanaPet - Update warehouse
            $object->getLinesArray();
            if ($object->check_warehouse($user) <= 0) {
                setEventMessages($langs->trans('ErrorValidate'), null, 'errors');
                $error++;
            } else {
         ...
         */
        if ($action == 'shipped') {
            $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' 
                    . $object->id, $langs->trans('CloseOrder')
                    , $langs->trans('ConfirmCloseOrder'), 'confirm_shipped', '', 0, 1);
        }
        ...
        // NanaPet - Config all layout for printer
        print '<table class="border" width="100%" id="border_print">';
        ...
        // Order customer
        print '<tr><td>' . $langs->trans('Customer_Order') . '</td>';
        print '<td colspan="3">' . $soc->getNomUrl(1) . '</td>';
        print '</tr>';

        if ($object->chk_received_order == '1') {
            print '<tr><td>' . $langs->trans('') . '</td>';
            print '<td><input type="checkbox" name="chk_received_order" id="chk_received_order" '
            . 'style="font-size: 110%" checked disabled/>';
            print '&nbsp'.$langs->trans("DefaultReceived");
            print '</td></tr>';
        } else {
            // Received Customer
            print '<tr><td>' . $langs->trans('ReceivedName') . '</td>';
            print '<td colspan="3">' . $object->recieved_name . '</td>';
            print '</tr>';

            // Received Phone
            print '<tr><td>' . $langs->trans('ReceivedPhone') . '</td>';
            print '<td colspan="3">' . $object->recieved_phone . '</td>';
            print '</tr>';

            // Received Address
            print '<tr><td>' . $langs->trans('ReceivedAddress') . '</td>';
            print '<td colspan="3">' . $object->recieved_address . '</td>';
            print '</tr>';
        }
        
        // NanaPet - Warehouse
        print '<tr><td>' . $langs->trans('Warehouse') . '</td>';
        print '<td colspan="3">' . $object->entrepot_label . " - ".
            $object->entrepot_lieu .'</td>';       
        print '<input type="text" colspan="3" style="display:none" '
            . 'class="entrepot_id" id ="entrepot_id" value="' 
            . $object->entrepot_id . '"></input>';
        print '</tr>';
        ...
        print '<tr><td>' . $langs->trans('DiscountMember') . '</td><td colspan="3">';
        $array_discount = $object_client->getDiscountOfCustomer($soc->email);
        $soc->remise_percent = $array_discount[0];        
        if($soc->remise_percent <= 0) {
            $soc->remise_percent = 0 ;
        }
        if ($soc->remise_percent) {
            // print $langs->trans("CompanyHasRelativeDiscount");
            $display_discount= $array_discount[0]."% - ". $array_discount[1];
            print "<span style='color:red'>" .$display_discount. "</span>";
        }        
        else {
            print $langs->trans("CompanyHasNoRelativeDiscount");
        }
        ...
        print '<td class="border" valign="top">' . $langs->trans('NoteOrder') . '</td>';
        print '<td valign="top" colspan="2">';
        print '<textarea rows="4" cols="100" disabled>';
        print $object->note_order;
        print '</textarea>';
        // print '<textarea name="note_public" wrap="soft" cols="70" rows="'.ROWS_3.'">'.$note_public.'</textarea>';
        print '</td></tr>';
        ...
        // Total TTC
        $total_money_order = price($object->total_ttc, 1, '', 1, - 1, - 1, $conf->currency);
        print '<tr><td>' . $langs->trans('TotalOrder') . '</td><td>' .'+ '. $total_money_order. '</td></tr>';
        
        // Ship
        $ship_order = price($object->ship, 1, '', 1, - 1, - 1, $conf->currency);
        print '<tr><td>' . $langs->trans('Ship') . '</td><td>' .'+ '.$ship_order. '</td></tr>';
        
        // Discount order
        $discount_order =  price($object->discount, 1, '', 1, - 1, - 1, $conf->currency);
        print '<tr><td>' . $langs->trans('DiscountOrder') . '</td><td>' .'- '.$discount_order. '</td></tr>';
        
        // Count total money
        $total_money =  $object->total_ttc + $object->ship - $object->discount;
        if ($total_money <= 0 ) {
            $total_money = 0;
        }
      
        // Total money
        $sum_price = price($total_money, 1, '', 1, - 1, - 1, $conf->currency);
        print '<tr><td>' . $langs->trans('TotalMoney') . '</td><td>' .$sum_price. '</td></tr>';
        ...
        // NanaPet - add bill
        $object->printBill($soc->name,$soc->phone,$soc->address, $object->chk_received_order,
                $object->recieved_name, $object->recieved_phone, $object->recieved_address,
                $object->note_order, $object->statut);
        print '<table id="tablelines" class="noborder noshadow" width="100%">';
        // NanaPet - Show object lines
         ...
         */
        // NanaPet - Add new line while optioncss != print
        if ($object->statut == Commande::STATUS_DRAFT && $user->rights->commande->creer 
                && GETPOST("optioncss") != 'print') {
            if ($action != 'editline') {
                $var = true;
        ...
        print '</table>';
        // NanaPet - add bill
        $object->printBillMoney($total_money_order, $ship_order, $discount_order
                                ,$sum_price);
        print "</form>\n";
        dol_fiche_end();
        ...
        if ($action != 'presend' && $action != 'editline') {
            // NanaPet - Change class name from 'tabsAction' to 'tabsAction_commande'
            print '<div class="tabsAction_print">';
            $parameters = array();
            ...
            if (empty($reshook)) {
                // Valid
                if ($object->statut == Commande::STATUS_DRAFT 
                    && $object->total_ttc >= 0 && $numlines > 0 
                    && ((empty($conf->global->MAIN_USE_ADVANCED_PRMS) 
                    && !empty($user->rights->commande->creer)) 
                    || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) 
                    && !empty($user->rights->commande->order_advance->validate)))) {
                    print '<div class="inline-block divButAction"><a class="butAction" href="' 
                    . $_SERVER["PHP_SELF"] . '?id=' . $object->id . '&amp;action=validate">' 
                    . $langs->trans('Validate') . '</a></div>';
                }
                // Edit
                if ($object->statut == Commande::STATUS_VALIDATED && $user->rights->commande->creer) {
                    // print '<div class="inline-block divButAction">
                    // <a class="butAction" href="card.php?id=' . $object->id . '&amp;action=modif">' 
                ...
                if (($object->statut == Commande::STATUS_CLOSED 
                        ...
                        && $object->facturee != "1" ) {
                    print '<div class="inline-block divButAction"><a class="butAction" href="' 
                        . $_SERVER['PHP_SELF'] . '?id=' . $object->id . '&amp;action=reopen">' 
                ...
                if ($conf->contrat->enabled && ($object->statut == Commande::STATUS_VALIDATED || $object->statut == Commande::STATUS_ACCEPTED)) {
                    ...
                    if ($user->rights->contrat->creer) {
                        print '<div class="inline-block divButAction"><a class="butAction" href="' 
                        . DOL_URL_ROOT . '/contrat/card.php?action=create&amp;origin=' . $object->element 
                        . '&amp;originid=' . $object->id . '&amp;socid=' . $object->socid . '">' . $langs->trans('AddContract') . '</a></div>';
                    }
                }
                ...
                // Clone
                if ($user->rights->commande->creer && $object->statut != Commande::STATUS_CLOSED) {
                    /*print '<div class="inline-block divButAction"><a class="butAction" href="' 
                    . $_SERVER['PHP_SELF'] . '?id=' 
                    . $object->id . '&amp;socid=' 
                    . $object->socid . '&amp;action=clone&amp;object=order">' 
                    . $langs->trans("ToClone") . '</a></div>';*/
                }

                // Cancel order
                if ($object->statut == Commande::STATUS_VALIDATED &&
                        ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->commande->cloturer)) || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->commande->order_advance->annuler)))
                ) {
        ...
        if ($action != 'presend') {
            ...
            // NanaPet - Don't show table print document in commande because of not supporting print UTF-8 document
