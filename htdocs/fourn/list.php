...
if (!empty($search_categ) || !empty($catid))
    $sql.= ' LEFT JOIN ' . MAIN_DB_PREFIX . "categorie_fournisseur as cf ON s.rowid = cf.fk_soc"; // We need this table joined to the select in order to filter by categ
...
if ($search_name)
    $sql .= natural_search(array('s.nom', 's.name_alias'), $search_name);
if ($search_zipcode)
...
if ($search_categ > 0)
    $sql.= " AND cf.fk_categorie = " . $search_categ;
if ($search_categ == -2)
    $sql.= " AND cf.fk_categorie IS NULL";
...
if ($resql) {
    ...
    print_liste_field_titre($langs->trans("CustomerName"), $_SERVER["PHP_SELF"], "s.nom", "", $param, 'valign="middle"', $sortfield, $sortorder);
