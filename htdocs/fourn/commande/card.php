...
if (empty($reshook)) {
    ...
    if ($action == 'setref_supplier' && $user->rights->fournisseur->commande->creer) {
        ...
        if ($result < 0)
            ...
            $object->ref_supplier = GETPOST('ref_supplier', 'alpha'); // The setValueFrom does not set new property of object
    }
    ...
    if ($action == 'reopen') { // no test on permission here, permission to use will depends on status
        if (in_array($object->statut, array(1, 2, 3, 5, 6, 7, 9))) {
            if ($object->statut == 1)
                ...
                $newstatus = 4; // Received->Received partially
            else if ($object->statut == 6)
                $newstatus = 2; // Canceled->Approved
            else if ($object->statut == 7)
                $newstatus = 3; // Canceled->Process running
            else if ($object->statut == 9)
        ...
        if ((GETPOST('prod_entry_mode') != 'free') && empty($error)) { // With combolist mode idprodfournprice is > 0 or -1. With autocomplete, idprodfournprice is > 0 or ''
            ...
            if ($idprod > 0) {
                ...
                $result = $object->addline(
                        $desc, $productsupplier->fourn_pu, $qty, $tva_tx, $localtax1_tx, $localtax2_tx, $productsupplier->id, GETPOST('idprodfournprice'), $productsupplier->fourn_ref, $remise_percent, 'HT', $pu_ttc, $type, '', '', $date_start, $date_end, $array_options, $productsupplier->fk_unit
                );
            }
    ...
    if ($action == 'updateline' && $user->rights->fournisseur->commande->creer && !GETPOST('cancel')) {
        ...
        $result = $object->updateline(
                $lineid, $_POST['product_desc'], $ht, $_POST['qty'], $_POST['remise_percent'], $tva_tx, $localtax1_tx, $localtax2_tx, $price_base_type, 0, isset($_POST["type"]) ? $_POST["type"] : $line->product_type, false, $date_start, $date_end, $array_options, $_POST['units']
        );
        unset($_POST['qty']);
        unset($_POST['type']);
        unset($_POST['idprodfournprice']);
    ...
    if ($action == 'confirm_valid' && $confirm == 'yes' &&
            ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->fournisseur->commande->creer)) || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->fournisseur->supplier_order_advance->validate)))
    ) {
                    ...
                    if ($object_id > 0) {
                        ...
                        if ($result > 0) {
                            ...
                            for ($i = 0; $i < $num; $i ++) {
                                ...
                                $result = $object->addline(
                                        $desc, $lines[$i]->subprice, $lines[$i]->qty, $lines[$i]->tva_tx, $lines[$i]->localtax1_tx, $lines[$i]->localtax2_tx, $lines[$i]->fk_product, $productsupplier->product_fourn_price_id, $productsupplier->ref_fourn, $lines[$i]->remise_percent, 'HT', 0, $lines[$i]->product_type, '', '', null, null, array(), $lines[$i]->fk_unit
                                );
...
if ($action == 'create') {
    ...
    if (!empty($conf->incoterm->enabled)) {
        ...
        if ($user->rights->fournisseur->commande->creer)
            print '<a href="' . DOL_URL_ROOT . '/fourn/commande/card.php?id=' . $object->id . '&action=editincoterm">' . img_edit() . '</a>';
        else
            print '&nbsp;';
        print '</td></tr></table>';
        print '</td>';
