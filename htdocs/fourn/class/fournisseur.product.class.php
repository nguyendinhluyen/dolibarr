...
class ProductFournisseur extends Product {
    ...
    function display_price_product_fournisseur($showunitprice = 1, $showsuptitle = 1, $maxlen = 0, $notooltip = 0) {
        ...
        $out = ($showunitprice ? formatMoney(price2num($this->fourn_unitprice)) . ' ' . $langs->trans("HT") . ' &nbsp; (' : '') . ($showsuptitle ? $langs->trans("Supplier") . ': ' : '') . $this->getSocNomUrl(1, 'supplier', $maxlen, $notooltip) . ' / ' . $langs->trans("SupplierRef") . ': ' . $this->fourn_ref . ($showunitprice ? ')' : '');
        return $out;
    }
