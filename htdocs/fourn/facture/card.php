                    ...
                    if ($result > 0) {
                        ...
                        for ($i = 0; $i < $num; $i++) {
                            ...
                            $result = $object->addline(
                                    $desc, $lines[$i]->subprice, $lines[$i]->tva_tx, $lines[$i]->localtax1_tx, $lines[$i]->localtax2_tx, $lines[$i]->qty, $lines[$i]->fk_product, $lines[$i]->remise_percent, $date_start, $date_end, 0, $lines[$i]->info_bits, 'HT', $product_type, $lines[$i]->rang, 1, $lines[$i]->array_options, $lines[$i]->fk_unit
                            );
    ...
    elseif ($action == 'updateline' && $user->rights->fournisseur->facture->creer) {
        ...
        if (GETPOST('prod_entry_mode') != 'free') { // With combolist mode idprodfournprice is > 0 or -1. With autocomplete, idprodfournprice is > 0 or ''
            ...
            if (GETPOST('idprodfournprice') == -1 || GETPOST('idprodfournprice') == '')
                $idprod = -2; // Same behaviour than with combolist. When not select idprodfournprice is now -2 (to avoid conflict with next action that may return -1)
...
if ($action == 'create') {
    ...
    if ($id > 0 || !empty($ref)) {
        ...
        if ($action != 'editdate_lim_reglement' && $object->statut < FactureFournisseur::STATUS_CLOSED && $object->date_echeance && $object->date_echeance < ($now - $conf->facture->fournisseur->warning_delay))
        ...
        if (!empty($conf->incoterm->enabled)) {
            ...
            if ($user->rights->fournisseur->facture->creer)
                print '<a href="' . DOL_URL_ROOT . '/fourn/facture/card.php?facid=' . $object->id . '&action=editincoterm">' . img_edit() . '</a>';
            else
                print '&nbsp;';
            print '</td></tr></table>';
            print '</td>';
            ...
            if (empty($reshook)) {
                ...
                if ($action != 'edit' && $object->statut == FactureFournisseur::STATUS_DRAFT) {
                    if (count($object->lines)) {
                        if ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->fournisseur->facture->creer)) || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->fournisseur->supplier_invoice_advance->validate))) {
