...
if ($search_ref) {
    if (is_numeric($search_ref))
        $sql .= natural_search(array('fac.ref'), $search_ref);
    else
...
if ($resql) {
    ...
    while ($i < min($num, $limit)) {
        ...
        if (($obj->paye == 0) && ($obj->fk_statut > 0) && $obj->date_echeance && $db->jdate($obj->date_echeance) < ($now - $conf->facture->fournisseur->warning_delay))
